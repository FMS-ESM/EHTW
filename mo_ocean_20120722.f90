...
!
  ...
  USE mo_control,         ONLY: nlon,ngl,                                             &
                                ...
                                nocn_sv,lwarning_msg,lsit,locn,locn_msg,              &
                                ocn_couple_option,ratio_dt_o2a,nproca,nprocb
  USE mo_netcdf,          ONLY: lkvl,nfnlvl,ocn_tlz,ocn_k0,ocn_z,diecast_zdepth      !  k1=30, nfnlvl=12, lkvl=39     : number of water layers                                
...
!!!                                MPI_INTEGER,MPI_STATUS_SIZE,p_all_comm,p_real_dp
  ...
  USE mo_time_control,    ONLY: delta_time, lstart, get_time_step, current_date,      &
                                ...
                                lreset_ocn_cpl, ltrigocn, ev_trigocn, lspinup                                                                
  ...
  REAL(dp), PARAMETER:: HBK0=1.34E-7_dp                     ! molecular heat diffusivity (m2/s) (=1.34E-3_dp cm2/s)
                                                            ! (Paulson and Simpson, 1981; Chia and pwu, 1998; Mellor and Durbin, 1975)  
  REAL(dp), PARAMETER:: DM0Z=5.E2_dp                        ! near shore momentum diffusivity (m2/s) for preventing generating  near-shore vortex
                                                            ! Noisy for current                                   
                                                            ! REAL(dp), PARAMETER:: DM0Z=1.E2_dp (seems to small)
                                                            ! REAL(dp), PARAMETER:: DM0Z=1.E5_dp (OK for current but diffuse too mush for depth at 150 m)                                   
                                                            !  
  ...
  LOGICAL:: ltest=.FALSE.                                   ! =.TRUE. for free surface, =.FALSE. for rigid lid,  
  ...
  LOGICAL, PARAMETER:: lopen_bound=.FALSE.                  ! .TRUE. = open boundary condition (set depth at j=jos0,jon0 according to its physical depth
...
!!!    !                                                       
    ...
    REAL(dp):: local_max_vel_water,max_vel_land,max_vel_water
...
!*******************************************************************
  SUBROUTINE set_ocn
    ...
    !!!REAL(dp):: go_lone=360._dp
    INTEGER:: i
    IF ( lwarning_msg.GE.2 ) WRITE(nerr,*) p_pe,"set_ocn 1: pe=",p_pe
    ...
    ELSE
      ...
      lnlat=lc%llnlat(1)
    ENDIF
    IF ( lwarning_msg.GE.2 ) WRITE(nerr,*) "pe=",p_pe,"lnlon=",lnlon,"lnlat=",lnlat
    IF ( lwarning_msg.GE.2 ) WRITE(nerr,*) "pe=",p_pe,"lc%pelonw0=",lc%pelonw0,"lc%pelone0=",lc%pelone0
    IF ( lwarning_msg.GE.2 ) WRITE(nerr,*) "pe=",p_pe,"lc%pelats0=",lc%pelats0,"lc%pelatn0=",lc%pelatn0
    IF ( lwarning_msg.GE.2 ) WRITE(nerr,*) "pe=",p_pe,"lc%iow0=",lc%iow0,"lc%ioe0=",lc%ioe0
    IF ( lwarning_msg.GE.2 ) WRITE(nerr,*) "pe=",p_pe,"lc%jos0=",lc%jos0,"lc%jon0=",lc%jon0
    IF ( lwarning_msg.GE.2 ) WRITE(nerr,1001) p_pe,lc%glats,lc%glate
    IF ( lwarning_msg.GE.2 ) WRITE(nerr,1002) p_pe,lc%glons,lc%glone
    ...
    CALL set_ocndepth2
    IF (lstart) THEN    
      IF ( lwarning_msg.GE.2 ) WRITE(nerr,*) p_pe,"set_ocn 5"   
      CALL pe_ocean_mesh
    ENDIF            
  CONTAINS
  !------------------------------------------------------
...
!
    ...
    dc%jgos0=jgosm1+1
    dc%igow0=igowm1+1
    IF ( lwarning_msg.GE.2 ) WRITE(nout,999) p_pe,dc%lLatPeriodical,dc%jgos0,dc%jgon0
    IF ( lwarning_msg.GE.2 ) WRITE(nout,1000) p_pe,dc%lLonPeriodical,dc%igow0,dc%igoe0
  ...
  ! ----------------------------------------------------------------------    
    ...
    IF ( lwarning_msg.GE.2 ) WRITE(nerr,*) p_pe,"meshgen_latitude 1.0"
    ! 2.0 Prepare latitude array
    IF ( lwarning_msg.GE.2 ) WRITE(nerr,*) "ocn_lon_factor=",ocn_lon_factor,"ocn_lat_factor=",ocn_lat_factor   
    IF ( lwarning_msg.GE.2 ) WRITE(nerr,*) "lc%nglat=",lc%nglat,"lc%nglon=",lc%nglon
    IF ( lwarning_msg.GE.2 ) WRITE(nerr,*) "jos0=",lc%jos0,"jon0=",lc%jon0,"ng=",ng
    DO ih=1,nh
...
!
      ...
      IF ( lwarning_msg.GE.2 ) WRITE(nerr,1013) "pe","ih","jo","jo2a","YVDEG","YDEG","philat"
      ...
      DO jo=1-ng,lnlat+ng
        ...
        IF ((ja.GE.1).AND.(ja.LE.lc%nglat/2)) THEN
          IF ( lwarning_msg.GE.2 ) WRITE(nerr,1014) p_pe,ih,jo,jo2a(jo,ih),YVDEG(jo,ih),YDEG(jo,ih),lphilat(ja,ih)
        ELSE
          IF ( lwarning_msg.GE.2 ) WRITE(nerr,1014) p_pe,ih,jo,jo2a(jo,ih),YVDEG(jo,ih),YDEG(jo,ih),xmissing
        ENDIF
        1014   FORMAT(4(I6,3X),3(F8.4,3X))  
      ENDDO
      IF ( (lc%set_b.EQ.1).AND.(lwarning_msg.GE.1) ) WRITE(nerr,20) p_pe,YDEG(1-ng:lnlat+ng,ih)
    ...
    IF ( lwarning_msg.GE.2 ) WRITE(nerr,*) p_pe,"meshgen_latitude 5.0"
  ...
  SUBROUTINE meshgen_longtitude
    ...
    IF ( lwarning_msg.GE.2 ) WRITE(nerr,*) "iow0=",lc%iow0,"ioe0=",lc%ioe0  
    IF ( lwarning_msg.GE.2 ) WRITE(nerr,*) 'i   io2a'
    DO ih=1,nh
      ...
      IF ( lwarning_msg.GE.2 ) WRITE(nerr,1013) "pe","ih","io","io2a","XVDEG","XDEG","philon"
      1013  FORMAT(4(A6,3X),3(A8,3X))
      DO io=1-ng,lnlon+ng
        ia=findi(XDEG(io,ih),lphilonb(0:lc%nglon,ih),0,lc%nglon,.FALSE.)
        IF (ia.EQ.int_missing) THEN
          io2a(io,ih)=ia      
          IF ( lwarning_msg.GE.2 ) WRITE(nerr,1014) p_pe,ih,io,io2a(io,ih),XVDEG(io,ih),XDEG(io,ih),xmissing
        ELSE
          io2a(io,ih)=ia      
          IF ( lwarning_msg.GE.2 ) WRITE(nerr,1014) p_pe,ih,io,io2a(io,ih),XVDEG(io,ih),XDEG(io,ih),lphilon(ia,ih)
        ENDIF
        1014  FORMAT(4(I6,3X),3(F8.4,3X))
      ENDDO
      IF ( lwarning_msg.GE.2 ) WRITE(nerr,1011) 'pe','ih','J ','jo2a ','YDEG ','YVDEG ','DX ','DY ','DY/DX'
...
!!!!          OCSV(J-1,ih)=1./CSV(J-1,ih)
        ...
        IF ( lwarning_msg.GE.2 ) WRITE(nerr,1012) p_pe,ih,j,jo2a(j,ih),YDEG(j,ih),YVDEG(j,ih),DX(j,ih)/1.E3,DY(j,ih)/1.E3,TEMP
        1012  FORMAT(4(I6,3X),5(F8.4,3X))        
      ENDDO
      IF ( lwarning_msg.GE.2 ) WRITE(nerr,20) p_pe,XDEG(1-ng:lnlon+ng,ih)
      IF ( lwarning_msg.GE.2 ) WRITE(nerr,21) p_pe,XVDEG(1-ng-1:lnlon+ng,ih)
...
!    
    ...
    TANPHI=0._dp
    IF ( lwarning_msg.GE.2 ) WRITE(nerr,*) p_pe,"allocate_ocn 2.0, ocn_couple_option=",ocn_couple_option,""
...
!    
    ...
    ia2o=0
    IF ( lwarning_msg.GE.2 ) WRITE(nerr,*) p_pe,"allocate_ocn 3.0"    
!    
  ...
  SUBROUTINE set_ocndepth2()
    IMPLICIT NONE     
    INTEGER:: jk
    IF ( p_parallel_ocean.AND.(lwarning_msg.GE.1) ) WRITE(nerr,4) (ocn_z(jk),jk=1,k01,nh)
    IF ( p_parallel_ocean.AND.(lwarning_msg.GE.1) ) WRITE(nerr,5) (ocn_z(jk),jk=2,k01,nh)
  !
  ! vertical metrics
  !
    DO jk=1,k1
...
!!!      mpi_pole=lc%mapmesh(lc%set_b,MOD(lc%set_a+nproca/2-1,nproca)+1)-1
    ...
    ELSE
      ...
      MPI_W=lc%mapmesh(lc%set_b-1,lc%set_a)-1
    ENDIF
    IF ( lwarning_msg.GE.2 ) WRITE (nerr,1001) p_pe,MPI_W,MPI_E,mpi_s,mpi_n,mpi_pole
    1001 FORMAT("pe=",I4,1X,"MPI_W=",(I4,1X),"MPI_E=",(I4,1X),"mpi_s=",2(I4,1X),"mpi_n=",2(I4,1X),"mpi_pole=",2(I4,1X))
...
!
  ...
  IF ( p_parallel_ocean.AND.(lwarning_msg.GE.1) ) THEN
    ...
    WRITE(nerr,*) p_pe,": I am in run_ocean 0.0: lspinup",lspinup
    WRITE (nerr,1106) p_pe,istep,ITF,INT(delta_time),INT(ratio_dt_o2a),INT(ocn_dt),INT(two_dt),INT(cpl_dt)
    1106  FORMAT('ocean: p_pe=',I5,', istep=',I8,', itf=',I8,', delta_time=',I8,', ratio_dt_o2a=',I8,', ocn_dt=',I8,', two_dt=',I8,', cpl_dt=',I8)
  ENDIF
  !!!  RETURN
  ...
  IF(lwarning_msg.GE.3) THEN
    IF (p_pe.EQ.3) THEN
      WRITE(nerr,*) p_pe,"run_ocean 1.0: entering"
      !!!      CALL ocn_msg()
    ENDIF
  ENDIF
!
...
!
  ...
  ENDIF
    ...
    IF(lwarning_msg.GE.2) then
      WRITE(nerr,*) p_pe,"run_ocean 1.11"
      !!! IF (p_pe==7) THEN
      !!!   WRITE(551) (REAL(gl_bathy(:,:)))
      !!!   CALL FLUSH(551)        
      !!!   WRITE(552) (REAL(gl_awust2(:,:)))
      !!!   CALL FLUSH(552)        
      !!!   WRITE(553) (REAL(gl_wtb(:,:)))      
      !!!   CALL FLUSH(553)        
      !!!   WRITE(554) (REAL(gl_wt(:,jk,:)),jk=1,lkvl+2)
      !!!   CALL FLUSH(554)        
      !!!   WRITE(555) (REAL(gl_wu(:,jk,:)),jk=1,lkvl+2)   
      !!!   CALL FLUSH(555)        
      !!!   WRITE(556) (REAL(gl_wv(:,jk,:)),jk=1,lkvl+2)   
      !!!   CALL FLUSH(556)        
      !!!   WRITE(557) (REAL(gl_ws(:,jk,:)),jk=1,lkvl+2)   
      !!!   CALL FLUSH(557)
      !!!   WRITE(558) (REAL(P(:,jk,:,1)),jk=1,lkvl+2)   
      !!!   CALL FLUSH(557)                
      !!! ENDIF
      !!!      WRITE(nerr,*) p_pe,"run_ocean 1.11, wtb=",gl_wtb
      !!!      WRITE(nerr,*) p_pe,"run_ocean 1.11, wt=",gl_wt
      !!!      WRITE(nerr,*) p_pe,"run_ocean 1.11, awust2=",gl_awust2 
...
!
    IF (lstart.AND.(ocn_couple_option.NE.13)) THEN
      ...
      ENDIF
      IF(lwarning_msg.GE.2) then
        WRITE(nerr,*) p_pe,"run_ocean 1.12"
        IF ( (p_pe.EQ.3).AND.(lwarning_msg.GE.2) ) THEN
          CALL ocn_msg("run_ocean 1.12")
        ENDIF
      ENDIF            
      IF (ocn_couple_option.EQ.11) THEN
        gl_wubm=gl_wub   
    ...
    ENDIF
    IF(lwarning_msg.GE.2) then
      WRITE(nerr,*) p_pe,"run_ocean 1.13"
      IF ( (p_pe.EQ.3).AND.(lwarning_msg.GE.2) ) THEN
        CALL ocn_msg("run_ocean 1.13")
      ENDIF
    ENDIF            
!
...
!
    ...
    ENDIF
    IF(lwarning_msg.GE.2) then
      WRITE(nerr,*) p_pe,"run_ocean 1.14"
      IF ( (p_pe.EQ.3).AND.(lwarning_msg.GE.2) ) THEN
        CALL ocn_msg("run_ocean 1.14")
      ENDIF
    ENDIF            
    IF (ocn_couple_option.EQ.11) THEN
    ...
    ENDIF
    IF(lwarning_msg.GE.2) then
      WRITE(nerr,*) p_pe,"run_ocean 1.15"
      IF ( (p_pe.EQ.3).AND.(lwarning_msg.GE.2) ) THEN
        CALL ocn_msg("run_ocean 1.15")
      ENDIF
    ENDIF
    ...
    IF(lwarning_msg.GE.2) then
      WRITE(nerr,*) p_pe,"run_ocean 1.2: before intpol_a2o"
      IF ( (p_pe.EQ.3).AND.(lwarning_msg.GE.2) ) THEN
...
!
    IF (lstart.OR.lspinup) THEN
      IF(lwarning_msg.GE.2) then
        WRITE(nerr,*) p_pe,"run_ocean 1.3: before intpol_a2o"
        IF ( (p_pe.EQ.3).AND.(lwarning_msg.GE.2) ) THEN
      ...
      ! CALL p_barrier(p_all_comm)  
      IF(lwarning_msg.GE.2) then
        WRITE(nerr,*) p_pe,"run_ocean 2.0: after intpol_a2o"
        IF ( (p_pe.EQ.3).AND.(lwarning_msg.GE.2) ) THEN
          CALL ocn_msg("run_ocean 2.0")
        ENDIF
      ENDIF
      CALL intpol_a2o(gl_wsb,SSIT(:,:,1,:),0._dp,1._dp,.FALSE.)                             ! both units are PSU, no unit convertion is needed
      ...
      ! feedback sit_ocean level 13 at 16.84 m depth to ocean level 2 data at 16.84 m
        CALL intpol_a2o(gl_wt(:,nfnlvl-1+jk,:),TSIT(:,:,jk,:),0._dp,1._dp,.FALSE.) ! convert from K to K
...
!!!        IF (louov.OR.ocn_couple_option.EQ.11) THEN
        IF (louov) THEN
          CALL intpol_a2o(gl_wu(:,nfnlvl-1+jk,:),USIT(:,:,jk,:),0._dp,1._dp,.FALSE.) ! convert from m/s to m/s
          CALL intpol_a2o(gl_wv(:,nfnlvl-1+jk,:),VSIT(:,:,jk,:),0._dp,1._dp,.FALSE.) ! convert from m/s to m/s
        ENDIF          
      ENDDO
    ELSE
    ! for getting true information of BC (ghost zone), setting lmiss=.TRUE.
      ...
      CALL intpol_a2o(gl_wsb,SSIT(:,:,1,:),0._dp,1._dp,.TRUE.)                            ! both units are PSU, no unit convertion is needed
      IF (ocn_couple_option.EQ.11) THEN
      ...
      ! feedback sit_ocean level 13 at 16.84 m depth to ocean level 2 data at 16.84 m
        ...
        CALL intpol_a2o(gl_ws(:,nfnlvl-1+jk,:),SSIT(:,:,jk,:),0._dp,1._dp,.TRUE.)  ! both units are PSU
            ! no unit convertion is needed
        IF (ocn_couple_option.EQ.11) THEN
          CALL intpol_a2o(gl_wu(:,nfnlvl-1+jk,:),USIT(:,:,jk,:),0._dp,1._dp,.TRUE.) ! convert from m/s to m/s
          CALL intpol_a2o(gl_wv(:,nfnlvl-1+jk,:),VSIT(:,:,jk,:),0._dp,1._dp,.TRUE.) ! convert from m/s to m/s
        ENDIF
      ENDDO
    ENDIF
!
...
!
    IF ((ocn_couple_option.EQ.0).OR.(ocn_couple_option.EQ.10)      &
      ...
      CALL intpol_a2o(gl_dwtbdt,TZSIT(:,:,1,:),0._dp,1._dp,.FALSE.)    ! both units are K/s, no unit convertion is needed
      CALL intpol_a2o(gl_dwsbdt,SZSIT(:,:,1,:),0._dp,1._dp,.FALSE.)    ! both units are PSU/s, no unit convertion is needed
      DO jk = 2, k1
      ! feedback sit_ocean level 13 at 16.84 m depth to ocean level 2 data at 16.84 m
...
!!!      SZSIT=(SSIT-S1)/cpl_dt  
    ELSEIF (ocn_couple_option.EQ.11) THEN
      CALL intpol_a2o(gl_dwtbdt,TZSIT(:,:,1,:),0._dp,1._dp,.FALSE.)    ! both units are K/s, no unit convertion is needed
      CALL intpol_a2o(gl_dwsbdt,SZSIT(:,:,1,:),0._dp,1._dp,.FALSE.)    ! both units are PSU/s, no unit convertion is needed
      CALL intpol_a2o(gl_dwubdt,UZSIT(:,:,1,:),0._dp,1._dp,.FALSE.)    ! both units are m/s/s, no unit convertion is needed
      CALL intpol_a2o(gl_dwvbdt,VZSIT(:,:,1,:),0._dp,1._dp,.FALSE.)    ! both units are m/s/s, no unit convertion is needed
      DO jk = 2, k1
      ! feedback sit_ocean level 13 at 16.84 m depth to ocean level 2 data at 16.84 m
        CALL intpol_a2o(gl_dwtdt(:,nfnlvl-1+jk,:),TZSIT(:,:,jk,:),0._dp,1._dp,.FALSE.)  ! K/s
        CALL intpol_a2o(gl_dwsdt(:,nfnlvl-1+jk,:),SZSIT(:,:,jk,:),0._dp,1._dp,.FALSE.)  ! PSU/s
...
!!!      VZSIT=(VSIT-V1)/cpl_dt
    ...
    ! CALL p_barrier(p_all_comm)
    IF ( lwarning_msg.GE.2 ) THEN    
      WRITE(nerr,*) p_pe,"run_ocean 3.0: after intpol_a2o"
      IF ( (p_pe.EQ.3).AND.(lwarning_msg.GE.2) ) THEN
...
!   
    IF (.TRUE.) THEN
      ...
      ! CALL p_barrier(p_all_comm)
      IF ( lwarning_msg.GE.2 ) THEN    
        WRITE(nerr,*) p_pe,"run_ocean 3.5: after ocn_stepon"
        IF ( (p_pe.EQ.3).AND.(lwarning_msg.GE.2) ) THEN
...
!    
    IF ( lwarning_msg.GE.2 ) THEN    
      WRITE(nerr,*) p_pe,"run_ocean 4.0 Feedback from DIECAST"
      IF ( (p_pe.EQ.3).AND.(lwarning_msg.GE.2) ) THEN
    ...
    IF ( lwarning_msg.GE.2 ) THEN
      ! CALL p_barrier(p_all_comm)    
      WRITE(nerr,*) p_pe,"run_ocean 4.05: write gl_wt,gl_wv,gl_wv,gl_ws"
    END IF    
    !!! IF ( (p_pe.EQ.3).AND.(lwarning_msg.GE.2) ) THEN    
    ...
    IF ( lwarning_msg.GE.2 ) THEN
      ! CALL p_barrier(p_all_comm)    
      WRITE(nerr,*) p_pe,"run_ocean 4.1: o2a: ocnt,ocnu,ocnv,ocnw"
    END IF
    IF ((lwarning_msg.GE.2).OR.locn_msg) THEN
      IF (.FALSE.) THEN
        DO ih=1,nh
...
!!!              CALL intpol_o2a(MERGE(DBLE(KVH2(:,:,jk,:)),xmissing,IW(:,:,jk+1,:).EQ.1),gl_ocnkvh(:,jk,:),0._dp,1._dp,2,ng)     ! from m2/s to m2/s
    ...
    IF(lwarning_msg.GE.2) then
      ! CALL p_barrier(p_all_comm)
      WRITE(nerr,*) p_pe,"run_ocean 5.0: Put data to individual nodes"
      IF ( (p_pe.EQ.3).AND.(lwarning_msg.GE.2) ) THEN
        CALL ocn_msg("run_ocean 5.0")
      ENDIF
    ENDIF
!
...
!  
  ...
  IF(lwarning_msg.GE.2) WRITE(nerr,*) p_pe,"run_ocean 6: leaving run_ocean"
  ...
  !
    ...
    IF ((two_dt_old.NE.two_dt).OR.lfirst_cycle) THEN
      IF (p_parallel_ocean) WRITE(nerr,*) "two_dt_old=",two_dt_old,"s, two_dt=",two_dt,"s"
    ...
    DO i_ocn_step=1, n_ocn_step
      ...
      IF ( lwarning_msg.GE.2 ) THEN
        ! CALL p_barrier(p_all_comm)
        WRITE(nerr,*) p_pe,"ocn_stepon 3.0: after FSGLO"    
      ENDIF
    ENDDO
    ...
    IF (p_parallel_ocean) WRITE(nerr,1107) istep,max_vel_land,max_vel_water,INT(ocn_dt)
    1107  FORMAT('istep=',I8,', max vel on land=',F8.4,', max vel on water=',F8.4,', ocn_dt=',I8,' s')
  ...
  !
    IF ( lwarning_msg.GE.2 ) THEN    
      ! CALL p_barrier(p_all_comm)
      WRITE(nerr,*) p_pe,"set_ocn_dt 1.0"
      !!! CALL ocn_msg()
    ENDIF
    ! ADAPATIVELY REMOVE NUMERICAL INSTABILITY
    IF (.TRUE.) THEN
      IF ((max_vel_water.GT.VMAX).OR.(acc_cpl_year.LE.0.1)) THEN
        ocn_dt=MIN(0.1_dp*ratio_dt_o2a*delta_time,cpl_dt)
      ELSEIF ( (max_vel_water .GT.(0.8_dp*VMAX)).OR.(acc_cpl_year.LE.0.2) ) THEN
        ocn_dt=MIN(0.2_dp*ratio_dt_o2a*delta_time,cpl_dt)
      ELSEIF ( (max_vel_water .GT.(0.7_dp*VMAX)).OR.(acc_cpl_year.LE.0.3) ) THEN
        ocn_dt=MIN(0.5_dp*ratio_dt_o2a*delta_time,cpl_dt)
      ELSEIF ( (max_vel_water .GT.(0.5_dp*VMAX)).OR.(acc_cpl_year.LE.0.5) ) THEN
  ...
  SUBROUTINE FSGLO
    ...
    !    IF ( lwarning_msg.GE.2 ) THEN
    !      CALL p_barrier(p_all_comm)
    !      WRITE(nerr,*) p_pe,"FSGLO 3.0: before set_ghost_all"    
    !    ENDIF
    ! --------------------------------
    IF ( lwarning_msg.GE.2 ) THEN
      ! CALL p_barrier(p_all_comm)
      IF ( (p_pe.EQ.3).AND.(lwarning_msg.GE.2) ) THEN
        WRITE(nerr,*) p_pe,"FSGLO 4.0: before timcom_transport"
        CALL ocn_msg("FSGLO 4.0")
      ENDIF       
    ENDIF
    DO ih=1,nh
      CALL timcom_transport(jpocn+nocntrac,u(:,:,:,ih),v(:,:,:,ih),w(:,:,:,ih),p(:,:,:,ih),        &
        in(:,:,:,ih),iu(:,:,:,ih),iv(:,:,:,ih),iw(:,:,:,ih),iu0(:,:,ih),iv0(:,:,ih),   &
        fb1(:,:,:,1:jpocn+nocntrac,ih),fblf(:,:,:,1:jpocn+nocntrac,ih),fbzsit(:,:,:,1:jpocn+nocntrac,ih),ih,fb2(:,:,:,1:jpocn+nocntrac,ih))
    ENDDO
    IF ( lwarning_msg.GE.2 ) THEN
      ! CALL p_barrier(p_all_comm)
      IF ( (p_pe.EQ.3).AND.(lwarning_msg.GE.2) ) THEN
    ...
    IF ( lwarning_msg.GE.2 ) THEN
      ! CALL p_barrier(p_all_comm)
      WRITE(nerr,*) p_pe,"FSGLO 10.0: before check_incompressibility"    
      IF ( (p_pe.EQ.3).AND.(lwarning_msg.GE.2) ) THEN
        CALL ocn_msg("FSGLO 10.0")
      ENDIF       
    ENDIF    
    ...
    P0=P0-PSM
    ! CALL p_barrier(p_all_comm)
    IF (p_parallel_ocean) WRITE(nerr,*) p_pe,"FSGLO, P0 10.1: PSM=",PSM
  
    IF ( lwarning_msg.GE.2 ) THEN
      ! CALL p_barrier(p_all_comm)
      WRITE(nerr,*) p_pe,"FSGLO 11.0: after check_incompressibility"    
      IF ( (p_pe.EQ.3).AND.(lwarning_msg.GE.2) ) THEN
  ...
  ! ----------------------------------------------------------------------
  SUBROUTINE timcom_transport(nq,u,v,w,p,in,iu,iv,iw,iu0,iv0,fb1,fblf,fbzsit,ih,fb2)
  ...
  !!!    REAL(dp):: PX(1-ng:lnlon+ng,1-ng:lnlat+ng),PY(1-ng:lnlon+ng,1-ng:lnlat+ng)    
      ...
      !!!    IF ( lwarning_msg.GE.2 ) THEN
      !!!      ! CALL p_barrier(p_all_comm)
      !!!      WRITE(nerr,*) p_pe,"timcom_transport 2.0"    
      !!!    ENDIF
      ! =================================================================
  ...
  SUBROUTINE open_boundary_conditions() 
   ...
   508      T2(I,J,1,ih)=T2(I,J,1,ih)+two_dt*W(I,J,1,ih)*T2(I,J,1,ih)*ODZ(1)
          ENDDO
        ENDDO
      ENDDO
  ...
  SUBROUTINE bottom_stress()
    INTEGER  ::I,J,K,ih
    REAL(dp) ::DRG,TMP
    REAL(dp), PARAMETER:: DRAGmax=2.e-2_dp       ! DRAG=drag coefficient (standard value is 2.e-1 dyne/cm^2=2.e-2 N/m2)   
  ! =============
  ! Bottom stress
  ! =============
    TMP=two_dt*DRAG
    DO ih=1,nh
      DO J=1,lnlat
        DO I=1,lnlon
          ...
          DRG=MIN(DRAGmax,IN(I,J,K,ih)*TMP*ODZ(K)*SQRT(U1(I,J,K,ih)**2+V1(I,J,K,ih)**2))
  ...
  SUBROUTINE EVP_solver()       
    ...
    ! SCR : delta velcoity (m/s)
      ...
      INTEGER:: I,J,K,M,N,L,LB,LT,ITMASK   
      INTEGER, PARAMETER:: MXMASK=18    ! MXMASK is maximum number of EVP iterations allowed
      !!! INTEGER, PARAMETER:: MXMASK=256    ! MXMASK is maximum number of EVP iterations allowed (v8.4a)
      REAL(dp), PARAMETER:: ocn_vtol=1.E-5_dp   ! maximum tolerance for water velocity over land (m/s)
                                                ! It was 0.01 cm/s (prior to v8.4).
      REAL(dp):: delta_uw_mx,delta_vw_mx,delta_vel_w_mx,ocn_vlmx,ocn_vlmx_old,ocn_vlmx_rr,ocn_umx,ocn_vmx     
      REAL(dp):: TEMP,TEMP1,TMP
      ...
      ! ---------------------------------------------------
      ! MXMASK is maximum number of iterations allowed
      !!!      MXMASK=18
      ...
      IF ( lwarning_msg.GE.3 ) THEN
        CALL p_barrier(p_all_comm)
        WRITE(nerr,*) p_pe,"EVP_solver 1.0"    
        IF ( (p_pe.EQ.3).AND.(lwarning_msg.GE.3) ) THEN
          CALL ocn_msg("EVP_solver 1.0")
        ENDIF       
      ENDIF      
      DO WHILE ( (ocn_vlmx.GT.ocn_vtol).AND.(ITMASK.LT.MXMASK).AND.(ocn_vlmx_rr.GT.0.05_dp) )
      !!!      DO WHILE (ocn_vlmx.GT..01_dp.AND.ITMASK.LT.MXMASK)
        ITMASK=ITMASK+1
  ...
  !
        ...
        IF ( lwarning_msg.GE.3 ) THEN
          CALL p_barrier(p_all_comm)
          WRITE(nerr,*) p_pe,"EVP_solver 2.0"    
          IF ( (p_pe.EQ.3).AND.(lwarning_msg.GE.3) ) THEN
            CALL ocn_msg("EVP_solver 2.0")
          ENDIF       
        ENDIF
        ! **************************************************************************   
        CALL P_BICGSTAB_LE(AB(:,:,:),AL(:,:,:),AC(:,:,:),AR(:,:,:),AT(:,:,:),   &
          DELS(:,:,:),DELX(:,:,:),CB(:,:,:),CL(:,:,:),CC(:,:,:),CR(:,:,:),CT(:,:,:), &
          lnlon,lnlat,nh,ITMASK)
        ! **************************************************************************   
        IF ( lwarning_msg.GE.3 ) THEN
          CALL p_barrier(p_all_comm)
          WRITE(nerr,*) p_pe,"EVP_solver 3.0"    
          IF ( (p_pe.EQ.3).AND.(lwarning_msg.GE.3) ) THEN
            CALL ocn_msg("EVP_solver 3.0")
          ENDIF       
        ENDIF      
        ...
        IF ( lwarning_msg.GE.3 ) THEN
          CALL p_barrier(p_all_comm)
          WRITE(nerr,*) p_pe,"EVP_solver 4.0"    
          IF ( (p_pe.EQ.3).AND.(lwarning_msg.GE.3) ) THEN
            CALL ocn_msg("EVP_solver 4.0")
          ENDIF       
        ENDIF
   ...
   677      V(1:lnlon,0:lnlat,K,:)=V(1:lnlon,0:lnlat,K,:)+SCR(1:lnlon,0:lnlat,2,:)*IV(1:lnlon,0:lnlat,K,:)
        ...
        IF ( p_parallel_ocean .AND. lwarning_msg.GE.2 ) THEN        
          WRITE(nerr,1001) p_pe,itmask,delta_vel_w_mx
        ENDIF
        1001 FORMAT(I4,'itmask=',I2,', delta_Vmx on water=',F9.5,' m/s')
        
        ! check convergence to zero flow over land
        ocn_vlmx_old=ocn_vlmx
        ocn_umx=MAXVAL((1-IU0(0:lnlon,1:lnlat,:))*ABS(U(0:lnlon,1:lnlat,1,:)))
        ocn_vmx=MAXVAL((1-IV0(1:lnlon,0:lnlat,:))*ABS(V(1:lnlon,0:lnlat,1,:)))  
        ocn_vlmx=MAX(ocn_umx,ocn_vmx)
        !!! IF (p_parallel_ocean) WRITE(nerr,684) p_pe,itmask,ocn_vlmx        
        684 FORMAT(I4,'itmask=',I2,', Vmx on land=',F9.5,' m/s')
        !>> le
        TMP=ocn_vlmx
        !!!! provide local max (TMP)
        CALL MPI_ALLREDUCE(TMP,ocn_vlmx,1,MPI_REAL8,MPI_MAX,p_all_comm,ierr)
        !!!! find global min (TEMP) for all nodes, and put it into TEMP  
        !<<
        ! improve solver accuracy (needed for stability)
        ocn_vlmx_rr=(ocn_vlmx_old-ocn_vlmx)/ocn_vlmx_old
        !!!      IF (ocn_vlmx.GT..01_dp.AND.ITMASK.LT.MXMASK) GO TO 652
        IF ( p_parallel_ocean .AND. lwarning_msg.GE.2 ) THEN
          WRITE(NERR,685) itmask,ocn_vlmx,ocn_vlmx_rr*100._dp
        ENDIF        
        685 FORMAT('global: ','itmask=',I2,', Vmx on land=',F9.5,' m/s, RR=',F6.2,'%')
      ENDDO
      ...
      IF ( lwarning_msg.GE.3 ) THEN
        CALL p_barrier(p_all_comm)
        WRITE(nerr,*) p_pe,"EVP_solver 6.0"    
        IF ( (p_pe.EQ.3).AND.(lwarning_msg.GE.3) ) THEN
          CALL ocn_msg("EVP_solver 6.0")
        ENDIF       
      ENDIF
      CALL calc_w()
      IF ( lwarning_msg.GE.3 ) THEN
        CALL p_barrier(p_all_comm)
        WRITE(nerr,*) p_pe,"EVP_solver 7.0"    
        IF ( (p_pe.EQ.3).AND.(lwarning_msg.GE.3) ) THEN
          CALL ocn_msg("EVP_solver 7.0")
        ENDIF       
      ENDIF
      CALL c2a(SCR)
      IF ( lwarning_msg.GE.3 ) THEN
        CALL p_barrier(p_all_comm)
        WRITE(nerr,*) p_pe,"EVP_solver 8.0"    
        IF ( (p_pe.EQ.3).AND.(lwarning_msg.GE.3) ) THEN
          CALL ocn_msg("EVP_solver 8.0")
        ENDIF
      ENDIF
  ...
  ! U2
      ...
      IF ( lwarning_msg.GE.3 ) THEN
        CALL p_barrier(p_all_comm)
        WRITE(nerr,*) p_pe,"c2a 2.0"    
        IF ( (p_pe.EQ.3).AND.(lwarning_msg.GE.3) ) THEN
          CALL ocn_msg("c2a 2.0")
        ENDIF
      ENDIF
      DO J=1,lnlat
        DO I=1,lnlon
  ...
  ! V2
    CALL exchangeNS3dReal(SCR(0:lnlon+1,-1:lnlat+1,4,:),1,lnlon,0,lnlat,1,nh,.FALSE.)
      IF ( lwarning_msg.GE.3 ) THEN
        CALL p_barrier(p_all_comm)
        WRITE(nerr,*) p_pe,"c2a 5.0"    
        IF ( (p_pe.EQ.3).AND.(lwarning_msg.GE.3) ) THEN
          CALL ocn_msg("c2a 5.0")
        ENDIF
      ENDIF
  ...
  ! Fourth order enhancement
      ...
      IF ( lwarning_msg.GE.2 ) THEN
        ! CALL p_barrier(p_all_comm)
        WRITE(nerr,*) p_pe,"c2a 6.0"    
        IF ( (p_pe.EQ.3).AND.(lwarning_msg.GE.2) ) THEN
          CALL ocn_msg("c2a 6.0")
        ENDIF
      ENDIF
    ...
    DO ih=1,nh
      ...
      !!! Smagorinsky, J. General circulation experiments with the primitive equations, I. The basic experiment. Monthly Weather Rev. 1963, 91, 99-164.
        DO K=1,k1
          DO J=1,lnlat
            DO I=0,lnlon
              DMX(I,J,K,ih)=0.12/(ODXV(j,ih)*ODY(j,ih))*SQRT(                                                            &
        ...
        DO K=1,k1
          DO J=0,lnlat
            DO I=1,lnlon   
              DMY(I,J,K,ih)=0.12/(ODX(j,ih)*ODYV(j,ih))*SQRT                                                             &
              (                                                                                                 &
      ...
      IF ( lwarning_msg.GE.2 ) THEN
        ! CALL p_barrier(p_all_comm)
        WRITE(nerr,*) p_pe,"PP82GLO 3.2"    
      ENDIF        
      IF (.TRUE.) THEN
        DO K=1,k1
          DO J=1,lnlat
            DO I=1,lnlon
              KHM(I,J,K,ih)=SQRT(((DMX(I-1,J,K,ih)+DMX(I-1,J,K,ih))/2._dp)**2+((DMY(I,J-1,K,ih)+DMY(I,J,K,ih))/2._dp)**2)
            ENDDO
          ENDDO
        ENDDO
        IF ( lwarning_msg.GE.2 ) THEN
          ! CALL p_barrier(p_all_comm)
          WRITE(nerr,*) p_pe,"PP82GLO 3.3"    
        ENDIF        
        CALL exchangeEW3dReal(KHM(:,:,:,ih),1,lnlon,1,lnlat,ng,k1)
        !!! DO J=lnlat+1,lnlat+ng
        !!!   KHM(:,J,:,ih)=DMY(:,lnlat,:,ih)  
        !!! ENDDO
        !!! DO J=1-ng,0
        !!!   KHM(:,J,:,ih)=DMY(:,1,:,ih)  
        !!! ENDDO
      ...
      IF ( lwarning_msg.GE.2 ) THEN
        ! CALL p_barrier(p_all_comm)
        WRITE(nerr,*) p_pe,"PP82GLO 4.0: after Smagorinsky"    
      ENDIF    
    ENDDO
...
!!!            CALL intpol_o2a(DBLE(P(:,:,1,:)),gl_wp(:,jkk,:),0._dp,1._dp,2,ng)      ! convert from N/m^2 to N/m^2 (Pa)
!!!            WRITE(nerr,*) p_pe,"run_ocean 4.249 Feedback from DIECAST"
          ENDDO
!!!          IF ( lwarning_msg.GE.2 ) THEN    
!!!            WRITE(nerr,*) p_pe,"run_ocean 4.25 Feedback from DIECAST"
!!!          ENDIF      
        ELSE
...
!!!      ENDIF
      IF ((ocn_couple_option.EQ.0).OR.(ocn_couple_option.EQ.10)          &
            .OR.(ocn_couple_option.EQ.11)                                &
            .OR.(ocn_coup.OR.(ocn_couple_option.EQ.11)                                &
            .OR.(ocn_couple_option.EQ.15).OR.(ocn_couple_option.EQ.16)   &
            .OR.(ocn_couple_option.EQ.17).OR.(ocn_couple_option.EQ.20)) THEN
        gl_wtbm=gl_wtb   
        gl_wsbm=gl_wsb   
...
!!!    ENDIF
    ...
    IF (.TRUE.) THEN
      ...
      DEALLOCATE (gl_depth)
! CALL p_barrier(p_all_comm)e Ocean Initial Data and EVP solver Initial Field
    !
    ! CALL p_barrier(p_all_comm)       
    ...
    W=0._dp
    TSIT=T2
    SSIT=S2! CALL p_barrier(p_all_comm) IF ( p_parallel_ocean.AND.(lwarning_msg.GE.2) ) THEN
      ! CALL p_barrier(p_all_comm)
      WRITE(nerr,*) p_pe,"ocn_ioinitial 5.0: leaving"
    ENDIF 
   !!! IF (p_parallel_ocean) WRITE(nerr,*) "IN(2:30,30,15,1)=",IN(2+1-2:30+1-2,30+1-2,15,1)
  ENDIF
  CONTAINS
  !---------------------------------------------------------------  
  SUBROUTINE PREP
    ...
    INTEGER:: irun,I,J,K,N,L,IT,NSUM,NN,ih
 ...
 10 FORMAT('Z-LEVELS(M)'/(10F8.2))
    ...
    IF ( lwarning_msg.GE.2 ) WRITE (nerr,1000) "p_pe","ih","j","TANPHI","F"
    ...
    DO ih=1,nh
      DO j=1,lnlat
        ...
        IF ( lwarning_msg.GE.2 ) WRITE (nerr,1001) p_pe,ih,j,TANPHI(J,ih),F(J,ih)
 ...
 14 FORMAT('LONGITUDE(X) GRID DIMENSION:',I4/    &
    ...
    IF ( lwarning_msg.GE.2 ) WRITE(nerr,*) p_pe,"PREP 2: CALL INMETS (reading etopo 5 data)"
    CALL INMETS(depth)    
    IF ( lwarning_msg.GE.2 ) WRITE (nerr,*) p_pe,"PREP 3: depth"
    DO ih=1,nh
      ...
      IF(.FALSE.)THEN
        ...
        DO K=1,k1
          ...
          depth(1:lnlon,N+1,ih)=MIN(depth(1:lnlon,N+1,ih),TM! CALL p_barrier(p_all_comm)
      ! bad point is (39,401) starting at step 91100
      ! CALL p_barrier(p_all_comm)
      IF ( lwarning_msg.GE.2 ) WRITE(nerr,24) (j,(int(depth(i,j,ih)),i=1,MIN(17,lnlon)),j=lnlat+ng,1-ng,-1)
      24 format((i3,4x,17I4))
      IF ( lwarning_msg.GE.2 ) WRITE (nerr,*) p_pe,"PREP 3.1"
      ...
      DO J=1-ng,lnlat+ng
        DO i=1-ng,lnlon+ng
         ...
         ! CALL p_barrier(p_all_comm),J,ih),TMP)
          ENDIF
        ENDDO
      ENDDO
      ! CALL p_barrier(p_all_comm)
      IF ( lwarning_msg.GE.2 ) WRITE (nerr,*) p_pe,"PREP 3.2"
    ENDDO
    IF (.NOT.lopen_bound) THEN
      ...
      ENDIF
    ENDIF
    depth=MIN(ocn_tlz,d! CALL p_barrier(p_all_comm)      ! PERIODIC
      WRITE (nerr,*) p_pe,"PREP 3.3"
      ! CALL p_barrier(p_all_comm)
      WRITE (nerr,*) p_pe,"PREP 3.31"    
      CALL exchangeEW3dR! CALL p_barrier(p_all_comm)nlat,ng,nh)
      WRITE (nerr,*) p_pe,"PREP 3.32"    
      ! CALL p_barrier(p_all_comm)
    ENDIF
    IF ( lwarning_msg.GE.2 ) WRITE (nerr,*) p_pe,"PREP 4."
    ...
    DO ih=1,nh
      DO K=1,k1
        ...
        ENDDO
      ENDDO
      IF ( lwarning_msg.GE.2 ) WRITE (nerr,*) p_pe,"PREP 4.1: max(KB), min(KB)"
      ! Derive land/sea mask array
      !!!      DO j=1,lnlat
      DO J=1-ng,lnlat+ng
        IF ( lwarning_msg.GE.2 ) WRITE(nerr,*) J,' KB',maxval(KB(:,J,ih)),minval(KB(:,J,ih))      
        CALL FLUSH(nerr)
        DO i=1-ng,lnlon+ng
          IT=KB(I,J,ih)
          IF (IT.NE.0) THEN
        ...
        ENDDO
      ENDDO
      IF ( lwarning_msg.GE.2 ) WRITE (nerr,*) p_pe,"PREP 4.2"
    ENDDO
    IF ( lwarning_msg.GE.2 ) WRITE (nerr,*) p_pe,"PREP 7"    
    ...
    IU! CALL p_barrier(p_all_comm):,:,1,:)=1
    IV0(:,:,:)=IV(:,:,1,:)
    IV(:,:,1,:)=1
    ! CALL p_barrier(p_all_comm)
    IF ( lwarning_msg.GE.2 ) WRITE (nerr,*) p_pe,"PREP 7.3: KB"    
    ...
    DO I=1-ng,lnlon+ng
     ! CALL p_barrier(p_all_comm)depth(I,J,:)=OCN_Z(2*KB(I,J,:)+1)
      ENDDO
    ENDDO
    ! CALL p_barrier(p_all_comm)
    IF ( lwarning_msg.GE.2 ) WRITE (nerr,*) p_pe,"PREP 7.4: KB"    
    !!! IF ( lwarning_msg.GE.2 ) WRITE(nerr,244) (ih,(j,(! CALL p_barrier(p_all_comm)lat+ng,1-ng,-1),ih=1,nh)
    244  format(2(I3,4x),17I4)
    ! CALL p_barrier(p_all_comm)
    IF ( lwarning_msg.GE.2 ) WRITE (nerr,*) p_pe,"PREP 7.5"     
    CALL intpo! CALL p_barrier(p_all_comm)ssing,IN(:,:,1,:).EQ.1),gl_depth(:,:),0._dp,1._dp,3,ng)
    ! CALL p_barrier(p_all_comm)
    IF ( lwarning_msg.GE.2 ) WRITE (nerr,*) p_pe,"I am leaving PREP 7.6"    
  ...
  END SUBROUTINE PREP
    ...
    ! ======================================================================
      ...
      INTEGER*2:: METERS_I2(NX,NY)
      INTEGER:: IN(1-ng:lnlon+ng,1-ng:lnlat+ng,nh)
      REAL(dp):: METERS(NX,NY)
      !!!      INTEGER*2 METERS(NX,NY),IN(1-ng:lnlon+ng,1-ng:lnlat+ng),METS(3361,1801)
      !!!   etopo 1
      !!!      INTEGER, PARAMETER:: NX=21600,NY=10800,NXPD=NX/360,NYPD=NY/180
      REAL(dp):: depth_ydeg(ny),depth_yvdeg(0:ny),depth_xdeg(nx),depth_xvdeg(0:nx) 
      LOGICAL:: PERI(2),RDIM1(2),RDIM2(2)
      ...
      REAL:: DT(NX,NY)                              ! for reading netcdf file 
	    ...
	    INTEGER:: I,ii,IMAX,IMIN,IP,J,jj,JP,K,M,N,NSEC,ih
      INCLUDE 'netcdf.inc'
      ...
      IF (p_parallel_ocean) THEN
!!!      IF (MYID .EQ. 0)  THEN
        IF ( lwarning_msg.GE.2 ) WRITE(nerr,*) p_pe,"INMETS 1: pe=",p_pe      
        IF (.TRUE.) THEN
...
!!!      REWIND 10
        ...
        ! New single record input
          READ(12) METERS_I2
          METERS=DBLE(METERS_I2)
          IF ( lwarning_msg.GE.2 ) WRITE(nerr,*) "METERS=",METERS(1,1)
        ELSE
        !!! etopo1 netcdf file
        !!!  ISTART(1) = 1
        !!!  ISTART(2) = 1
        ...
        ENDIF
      ENDIF
      IF ( lwarning_msg.GE.2 ) WRITE(nerr,*) p_pe,"INMETS 2: pe=",p_pe
      CALL p_bcast(METERS,p_ocean)
      !!! CALL MPI_BCAST(METERS,NX*NY,MPI_REAL,0,p_all_comm,ierr)
      IF ( lwarning_msg.GE.2 ) WRITE(nerr,*) p_pe,"INMETS 3: pe=",p_pe,"METERS=",METERS(1,1)
      ! ==============================================================
      ...
      DO ih=1,nh
        ...
        DO jj=1-ng,lnlat+ng
          ...
          DO ii=1-ng,lnlon+ng
            ...
            ! note: negative meters values mean positive ocean depths
            DNW=findGhostValue(i,jp,METERS(:,:),1,nx,1,ny)
            DNE=findGhostValue(ip,jp,METERS(:,:),1,nx,1,ny)
            DSW=findGhostValue(i,j,METERS(:,:),1,nx,1,ny)
            DSE=findGhostValue(ip,j,METERS(:,:),1,nx,1,ny)  
            !!! DSW=METERS(I,JP)
            !!! DSE=METERS(IP,JP)
            !!! DNW=METERS(I,J)
            !!! DNE=METERS(IP,J)
 ...
 101    ! CALL p_barrier(p_all_comm)_tlz) N=N+1
          ENDDO
        ENDDO
      ENDDO
      ! CALL p_barrier(p_all_comm)
      IF ( lwarning_msg.GE.2 ) WRITE(nerr,*) p_pe,"INMETS 3.3: pe=",p_pe,"lnlon=",lnlon,"lnlat=",lnlat
 102  IF ( lwarning_msg.GE.2 ) WRITE(nerr,103) p_pe,M,N,ocn_tlz,TMP
...
!*******************************************************************
  SUBROUTINE set_tsevp
    ...
    !
    IF ( lwarning_msg.GE.2 ) WRITE(nerr,*) p_pe,"set_tsevp 1.0"
    !!! IF(p_pe.EQ.3.AND.lwarning_msg.GE.2) THEN
    !!!   CALL ocn_msg("set_tsevp 1.0")
    !!! ENDIF
    CALL BOUNDS
!!!    IF (LEVP.NE.1) stop 1
    IF ( lwarning_msg.GE.2 ) WRITE(nerr,*) p_pe,"set_tsevp 2.0"
    ...
    DO ih=1,nh
      DO K=1,k1
        ...
        DO J=1,lnlat
          ...
          IF ( lwarning_msg.GE.2 )  WRITE(nerr,1001) p_pe,ih,k,j,DZX,DZYB,DZYT
 ...
 282            AT(I,J,ih)=AT(I,J,ih)+DZYT*IV(I,J,K,ih)
      ...
      ENDDO
    ENDDO
    IF ( lwarning_msg.GE.2 ) WRITE(nerr,*) p_pe,"set_tsevp 7.0"
...
!     AB(lnlon,1,:)=AB(lnlon,2,:)
 ...
 ! CALL p_barrier(p_all_comm)   AB=-AB
      AT=-AT
      AC0=-AL-AR-AB-AT
    ENDIF
    ! CALL p_barrier(p_all_comm)
    IF ( lwarning_msg.GE.2 ) WRI! CALL p_barrier(p_all_comm).0"
    IF ( lwarning_msg.GE.2 ) WRITE(nerr,*) "AL=",AL
    ! CALL p_barrier(p_all_comm)
    IF ( lwarning_msg.GE.2 ) WRITE(nerr,*) p_pe,"set_tsevp 9.0: leaving."
...
SUBROUTINE ocn_init
  ...
  ! ----------------------------------------------------------------------
    IMPLICIT NONE
    INTEGER, PARAMETER:: KTRM=5     ! KTRM=thermocline level (used only for animation graphics) 
    INTEGER:: I,J,K,M,N,NM,MP,N2,N4,N6,N8,ii,jj,ih
  ...
  ! is weak
    DO ih=1,nh
      DO K=1,k1
        !!!        IF (K.GT.KTRM) DM0Z=MIN(1.E6,1.1*DM0Z)
        ...
        ! Strong filter
        !       CARPET=16.*DM0Z*(1.-EXP(-OCN_Z(2*K)/OCN_Z(2*KTRM)))
        !       TMP=0.25
        ! Moderate filter
        ! Smoothly increase carpet filter from surface to bottom
        CARPET=4.*DM0Z*(1.-EXP(-OCN_Z(2*K)/OCN_Z(2*KTRM)))
        TMP=0.5
        DO j=1,lnlat
          DO I=1,lnlon-1
...
! ----------------------------------------------------------------------
    IMPLICIT NONE
    REAL(dp)::AL(lnlon,lnlat),AR(lnlon,lnlat),AB(lnlon,lnlat),AC(lnlon,lnlat), &
      AT(lnlon,lnlat),CL(lnlon,lnlat),CR(lnlon,lnlat),CB(lnlon,lnlat),CC(lnlon,lnlat), &
      CT(lnlon,lnlat),ACT(lnlon,lnlat)
    INTEGER:: i,j
...
SUBROUTINE BOUNDS
  ...
  !
  IF ( lwarning_msg.GE.2 ) WRITE(nerr,*) p_pe,"BOUNDS 1.0: read initial T and S data (CALL indata)"
  CALL indata      
  IF ( lwarning_msg.GE.2 ) WRITE(nerr,*) p_pe,"BOUNDS 1.1"      
  ...
  DO ih=1,nh
    ...
    IF ( lwarning_msg.GE.2 ) WRITE(nerr,*) p_pe,"BOUNDS 1.4"  
...
!!  << bjt
    DO K=2,k1
      ...
      DO J=1,lnlat
        ...
        ENDDO
      ENDDO
      IF ( lwarning_msg.GE.2 ) WRITE(nerr,58) p_pe,N,K
      58 FORMAT(I6,I6,' unstable points at level',I3)
    ENDDO
    IF ( lwarning_msg.GE.2 ) WRITE(nerr,*) p_pe,"BOUNDS 1.5"
    IF (.FALSE.) THEN  
    ! PERIODIC (no need anymore)
      CALL exchangeEW3dReal(T1(:,:,:,ih),1,lnlon,1,lnlat,ng,k1)  
      CALL exchangeEW3dReal(S1(:,:,:,ih),1,lnlon,1,lnlat,ng,k1)
      IF ( lwarning_msg.GE.2 ) WRITE(nerr,*) p_pe,"BOUNDS 2.0"
    ENDIF
  ENDDO
  CONTAINS
  !---------------------------------------------------------------
...
!
    ...
    IF ( lwarning_msg.GE.2 ) WRITE (nerr,*) p_pe,"indata 1.0."
...
! ==================================================
!CALL p_barrier(p_all_co! CALL p_barrier(p_all_comm)IF (p_parallel_ocean) THEN
      IF ( lwarning_msg.GE.2 ) WRITE (nerr,*) p_pe,"indata 2.0."
      IF (.TRUE.) CALL woa_season_4D_rd
      IF ( lwarning_msg.GE.2 ) WRITE (nerr,*) p_pe,"indata 3.0."
! ===================================================
...
!!!    DO M=dt_start(2),dt_start(2)                        ! for start month
      REWIND 41
      IF ( lwarning_msg.GE.2 ) WRITE(nerr,*) "month=",M
...
!!!      WRITE (nerr,*) p_pe,"indata 3.01. M=",M
      DO K=1,nlev_levitus
        ...
        IF ( lwarning_msg.GE.2 ) WRITE (nerr,*) p_pe,"indata 3.02. M=",M,", K=",K
        IF (.TRUE.) THEN
#ifdef __ibm__
          ...
          SDBL(:,:,K)=DBLE(SLEV(:,:,LT))
#endif        
          IF ( lwarning_msg.GE.2 ) WRITE (nerr,*) p_pe,"indata 3.03. M=",M,", K=",K
        ELSE
#ifndef __ibm__        
          ...
          IF ( lwarning_msg.GE.2 ) WRITE (nerr,*) p_pe,"indata 3.04. M=",M,", K=",K
#endif
        ENDIF
        DO J=1,nlat_levitus
          DO I=1,nlon_levitus
          ...
          ENDDO
        ENDDO
        IF ( lwarning_msg.GE.2 ) WRITE (nerr,*) p_pe,"indata 3.05. M=",M,", K=",K
        CALL swap(LT,LB)
      ENDDO
      ...
      CLOSE(40)
      !
      IF ( lwarning_msg.GE.2 ) WRITE (nerr,*) p_pe,"indata 3.1."
      CALL fix_negative_stratification_areas(TDBL,SDBL,ZLEV,nlon_levitus,nlat_levitus,nlev_levitus)          
      IF ( lwarning_msg.GE.2 ) WRITE (nerr,*) p_pe,"indata 3.2."
    ENDIF
    ! CALL p_barrier(p_all_! CALL p_barrier(p_all_comm)GE.2 ) WRITE (nerr,*) p_pe,"indata 4.0."
    CALL p_bcast(TDBL,p_ocean)
    CALL p_bcast(SDBL,p_ocean)
    ! =================================================
    ! Interpolate Levitus data to model horizontal grid
    ! =================================================
    IF ( lwarning_msg.GE.2 ) WRITE(nerr,*) p_pe,'interpolate Levitus data to model horizontal grid'
    DO ih=1,nh
      DO K=1,nlev_levitus
        XINC=DXMNUT/60.
        DO jj=1-ng,lnlat+ng
...
!!!            DSE=SDBL(IP,J,K)
      ...
      DO J=1,lnlat
        ...
        ENDDO
      ENDDO
      ! CALL p_barrier(p_all_! CALL p_barrier(p_all_comm)g_msg.GE.2 ) WRITE (nerr,*) p_pe,"indata 5.0."
      WRITE (nerr,245) p_pe,M,TMIN,TMAX
      245  FORMAT(I4,'month',I3,' Tmin,Tmax=',2(F6.2,1X))
      CALL FLUSH(nerr)
      ! CALL p_barrier(p_all_! CALL p_barrier(p_all_comm)g_msg.GE.2 ) WRITE (nerr,*) p_pe,"indata 5.1."
      CALL FLUSH(nerr)
      ! ===============================================
 ...
 322    C1=(Z2-ZC)/(Z2-Z1)
        C2=(ZC-Z1)/(Z2-Z1)
        IF ( lwarning_msg.GE.2 ) WRITE(nerr,323) p_pe,K,N
...
!     !!!    CALL ocn_msg()
      !!! ENDIF
      ! CALL p_barrier(p_all_! CALL p_barrier(p_all_comm)ing_msg.GE.2 ) WRITE (nerr,*) p_pe,"indata 5.2."
    ENDDO
    ! CALL p_barrier(p_all_! CALL p_barrier(p_all_comm)g_msg.GE.2 ) WRITE (nerr,*) p_pe,"indata 5.3."    
    DEALLOCATE (TDBL)
    DEALLOCATE (SDBL)
    DEALLOCATE (ZLEV)
    IF ( lwarning_msg.GE.2 ) THEN
      ! CALL p_barrier(p_all_! CALL p_barrier(p_all_comm)*) p_pe,"I am leaving indata 6."
    ENDIF
...
!!!  IF (p_pe.EQ.3) THEN  
!!!    IF ( lwarning_msg.GE.2 ) THEN
!!!      WRITE(nerr,*) p_pe,"cleanup_ocean: 1.0"
!!!      WRITE(nerr,*) "io2a=",io2a
!!!    ENDIF    
    !
    ...
    DEALLOCATE (io2a)
!!!    IF ( lwarning_msg.GE.2 ) THEN
!!!      WRITE(nerr,*) p_pe,"cleanup_ocean: 1.1"
!!!      WRITE(nerr,*) "jo2a=",jo2a
!!!    ENDIF    
    DEALLOCATE (jo2a)
!!!    IF ( lwarning_msg.GE.2 ) THEN
!!!      WRITE(nerr,*) p_pe,"cleanup_ocean: 1.2"
!!!      WRITE(nerr,*) "ja2o=",ja2o
!!!    ENDIF    
    DEALLOCATE (ja2o)
    ...
    !    
!!!    IF ( lwarning_msg.GE.1 ) THEN
!!!      WRITE(nerr,*) p_pe,"cleanup_ocean 2.0"
!!!      WRITE(nerr,*) "ODZ=",ODZ
!!!    ENDIF
    ...
    DEALLOCATE (XVDEG)
    DEALLOCATE (XDEG)
    IF ( lwarning_msg.GE.2 ) WRITE(nerr,*) p_pe,"cleanup_ocean 3.1"
    DEALLOCATE (CS)
    DEALLOCATE (CSV)
    DEALLOCATE (OCS)
!!!!    DEALLOCATE (OCSV)
    IF ( lwarning_msg.GE.2 ) WRITE(nerr,*) p_pe,"cleanup_ocean 3.2"
    
    DEALLOCATE (DX)
    IF ( lwarning_msg.GE.2 ) WRITE(nerr,*) p_pe,"cleanup_ocean 3.2.1"
    DEALLOCATE (DXV)
    IF ( lwarning_msg.GE.2 ) WRITE(nerr,*) p_pe,"cleanup_ocean 3.2.2"
    DEALLOCATE (ODX)
    IF ( lwarning_msg.GE.2 ) WRITE(nerr,*) p_pe,"cleanup_ocean 3.2.3"
    DEALLOCATE (ODXV)
    IF ( lwarning_msg.GE.2 ) WRITE(nerr,*) p_pe,"cleanup_ocean 3.3"
    
    DEALLOCATE (DY)
    DEALLOCATE (DYV)
    DEALLOCATE (ODY)
    DEALLOCATE (ODYV)
    IF ( lwarning_msg.GE.2 ) WRITE(nerr,*) p_pe,"cleanup_ocean 4.0"
    !  
    ! "A" grid arrays
    !
    IF ( lwarning_msg.GE.2 ) WRITE(nerr,*) p_pe,"cleanup_ocean 5.0"
    ! Logical depth KB and associated masking arrays
    ! Lower precision logical depth and masking arrays
    ! "A" grid arrays
    !
    ...
    IF ( lwarning_msg.GE.2 ) WRITE(nerr,*) p_pe,"cleanup_ocean: 6.0"
    ...
    !!!   DEALLOCATE (ADD)
    IF ( lwarning_msg.GE.2 ) THEN
      ! CALL p_barrier(p_all_! CALL p_barrier(p_all_comm)) p_pe,"cleanup_ocean: 7.0"
    ENDIF   
    DEALLOCATE (F)
    IF ( lwarning_msg.GE.2 ) THEN
      ! CALL p_barrier(p_all_! CALL p_barrier(p_all_comm)) p_pe,"cleanup_ocean: 7.1"
    ENDIF    
    DEALLOCATE (TANPHI)
           
    !C-grid arrays
    IF ( lwarning_msg.GE.2 ) THEN
      ! CALL p_barrier(p_all_! CALL p_barrier(p_all_comm)) p_pe,"cleanup_ocean: 7.2"
    ENDIF
     DEALLOCATE (IU0)
    IF ( lwarning_msg.GE.2 ) THEN
      ! CALL p_barrier(p_all_! CALL p_barrier(p_all_comm)) p_pe,"cleanup_ocean: 7.3"
    ENDIF    
    DEALLOCATE (IU)
    IF ( lwarning_msg.GE.2 ) THEN
      ! CALL p_barrier(p_all_! CALL p_barrier(p_all_comm)) p_pe,"cleanup_ocean: 7.4"
    ENDIF    
    DEALLOCATE (U)
    
    IF ( lwarning_msg.GE.2 ) THEN
      ! CALL p_barrier(p_all_! CALL p_barrier(p_all_comm)) p_pe,"cleanup_ocean: 7.5"
    ENDIF
        
    DEALLOCATE (IV0)
    IF ( lwarning_msg.GE.2 ) THEN
      ! CALL p_barrier(p_all_! CALL p_barrier(p_all_comm)) p_pe,"cleanup_ocean: 7.6"
    ENDIF        
    DEALLOCATE (IV)
    IF ( lwarning_msg.GE.2 ) THEN
      ! CALL p_barrier(p_all_! CALL p_barrier(p_all_comm)) p_pe,"cleanup_ocean: 7.7"
    ENDIF
    DEALLOCATE (V)
    
    IF ( lwarning_msg.GE.2 ) THEN
      WRITE(nerr,*) p_pe,"cleanup_ocean: 7.8"
    ENDIF            
    DEALLOCATE (IW)
    
    IF ( lwarning_msg.GE.2 ) THEN
      ! CALL p_barrier(p_all_! CALL p_barrier(p_all_comm)pe,"cleanup_ocean: 7.9"
    ENDIF
    DEALLOCATE (W)
    
    IF ( lwarning_msg.GE.2 ) THEN
      WRITE(nerr,*) p_pe,"cleanup_ocean: 8.0"
    ENDIF
    ...
    !
    IF ( lwarning_msg.GE.2 ) THEN
      ! CALL p_barrier(p_all_! CALL p_barrier(p_all_comm)pe,"cleanup_ocean: 9.0"
    ENDIF
    ...
    IF ( lwarning_msg.GE.2 ) THEN
      ! CALL p_barrier(p_all_! CALL p_barrier(p_all_comm)pe,"cleanup_ocean: 10.0"
    ENDIF
    !   
    DEALLOCATE (DMX)
    DEALLOCATE (DMY)
...
!!!    DEALLOCATE (KVH2)
    ...
    IF ( lwarning_msg.GE.2 ) THEN
      ! CALL p_barrier(p_all_! CALL p_barrier(p_all_comm)) p_pe,"cleanup_ocean: 11.0"
    ENDIF  
...
!------------------------------------------------------
  ...
  INTEGER:: LB,LT
  write(nerr,*) p_pe,"fix_negative_stratification_areas 1.0."
  DO J=1,j0
    DO I=1,i0
...
!!!    write(nerr,*) ZLEV(K)
  ...
  ENDDO 
  INSUM=(k1-1)*i0*j0
  WRITE(nerr,151) N,INSUM
  write(nerr,*) "I am leaving fix_negative_stratification_areas 9.0."
...
! ri = s ? £sit
! ----------------------------------------------------------------------
  SUBROUTINE P_BICGSTAB_LE(AB,AL,AC,AR,AT,B,X,CB,CL,CC,CR,CT,lnlon,lnlat,nh,ITMASK)
    ...
    INTEGER, INTENT(IN):: ITMASK                                       ! number of iteration 

    INTEGER, PARAMETER:: PC1maxIter=50                                 ! = 10000 (default)
    INTEGER, PARAMETER:: PC3maxIter=200                                ! = 10000 (default)
...
!      IF ( lwarning_msg.GE.2 ) THEN
!        CALL p_barrier(p_all_comm)
!        WRITE(nerr,*) p_pe,"P_BICGSTAB_LE 4.0"    
!        IF ( (p_pe.EQ.3).AND.(lwarning_msg.GE.2) ) THEN
!          CALL ocn_msg()
!        ENDIF       
!      ENDIF 
...
!        IF ( lwarning_msg.GE.2 ) THEN
!          ! CALL p_barrier(p_all_! CALL p_barrier(p_all_comm)*) p_pe,"P_BICGSTAB_LE 8.0, rhn",rhn              
!          WRITE(nerr,*) p_pe,"P=",P
!          WRITE(nerr,*) p_pe,"w=",w
!          WRITE(nerr,*) p_pe,"V=",V
!          IF ( (p_pe.EQ.3).AND.(lwarning_msg.GE.2) ) THEN
...
!!!        CALL DAXPY(I0J0,1._dp,R,1,P,1)            ! P=1*R+P
        ...
        P(:,:,:)=R(:,:,:)+beta*(P(:,:,:)-w*V(:,:,:))
!        IF ( lwarning_msg.GE.2 ) THEN
!          ! CALL p_barrier(p_all_! CALL p_barrier(p_all_comm)*) p_pe,"P_BICGSTAB_LE 8.1"    
!          IF ( (p_pe.EQ.3).AND.(lwarning_msg.GE.2) ) THEN
!            CALL ocn_msg()
!          ENDIF       
!        ENDIF 
...
!        CALL P_SIPSQELIM(CB,CL,CC,CR,CT,P,PH)
        ...
        alpha=rhn/tmp
!        IF ( lwarning_msg.GE.2 ) THEN
!          ! CALL p_barrier(p_all_! CALL p_barrier(p_all_comm)*) p_pe,"P_BICGSTAB_LE 8.2"    
!          IF ( (p_pe.EQ.3).AND.(lwarning_msg.GE.2) ) THEN
!            CALL ocn_msg()
!          ENDIF       
!        ENDIF       
...
!        IF ( lwarning_msg.GE.2 ) THEN
!          ! CALL p_barrier(p_all_! CALL p_barrier(p_all_comm)*) p_pe,"P_BICGSTAB_LE 8.3"    
!          IF ( (p_pe.EQ.3).AND.(lwarning_msg.GE.2) ) THEN
!            CALL ocn_msg()
!          ENDIF       
!        ENDIF       
...
!        IF ( lwarning_msg.GE.2 ) THEN
!          ! CALL p_barrier(p_all_! CALL p_barrier(p_all_comm)*) p_pe,"P_BICGSTAB_LE 9.0"    
!          IF ( (p_pe.EQ.3).AND.(lwarning_msg.GE.2) ) THEN
!            CALL ocn_msg()
!          ENDIF       
!        ENDIF 
...
!        PRINT*,ITER,'WN=',w
        ...
        rho=rhn
        IF ( p_parallel_ocean .AND. lwarning_msg.GE.2 ) THEN
          WRITE(nerr,1001) istep,ITMASK,ITER,ipc,res,res/res0*100._dp
        ENDIF
...
!        STOP
      ...
      END DO
      ipc=1
      ! CALL p_barrier(p_all_! CALL p_barrier(p_all_comm)ME()
      CALL set_ghost_3d (X,lnlon,lnlat,1,nh,.FALSE.)
    CONTAINS  
! ----------------------------------------------------------------------
...
!!!      COMMON/TIMER/TIMER(10)
 ...
 100    B(I,J,ih)=AB(i,j,ih)*X(I,j-1,ih)+AL(i,j,ih)*X(i-1,J,ih)+AC(i,j,ih)*X(I,J,ih) &
      ...
      ENDDO
      CALL set_ghost_3d (B(:,:,:),lnlon,lnlat,1,nh,.FALSE.) 
...
!!!    REAL(dp), INTENT(in out):: a(iow0-ing:,:)
    ...
    INTEGER:: my_mpi_vec                   ! MPI vector for 3-dimansional Real*4 Martix which nomal vector toward to East or West     
    lnlat=jon0-jos0+1
    lnlon=ioe0-iow0+1
...
!!!    a(iow0-ing:iow0-1,:)=a(ioe0-ing+1:ioe0,:)
!!!    a(ioe0+1:ioe0+ing,:)=a(iow0:iow0+ing-1,:)
  END SUBROUTINE exchangeEW3dReal
  !------------------------------------------------------
!!!  SUBROUTINE exchangeEW3dReal(a,iow0,ioe0,jos0,jon0,ng,k1)
!!!    REAL(dp), INTENT(in out):: a(iow0-ing:,:,:)
!!!    INTEGER, INTENT(in):: iow0,ioe0,jos0,jon0,ng,k1
!!!    a(iow0-ing:iow0-1,:,:)=a(ioe0-ing+1:ioe0,:,:)
!!!    a(ioe0+1:ioe0+ing,:,:)=a(iow0:iow0+ing-1,:,:)
!!!  END SUBROUTINE exchangeEW3dReal
!------------------------------------------------------
...
!------------------------------------------------------
  SUBROUTINE exchangeEW3dInt(a,iow0,ioe0,jos0,jon0,ng,k1)
!    INTEGER, INTENT(in out):: a(iow0-ing:,:,:)
    INTEGER, INTENT(in):: iow0,ioe0,jos0,jon0,ng,k1
    INTEGER, INTENT(in out):: a(iow0-ng:ioe0+ng,jos0-ng:jon0+ng,k1)
!!!    REAL(dp), INTENT(in out):: a(iow0-ing:,:)
    INTEGER:: lnlat,lnlon
    INTEGER:: my_mpi_vec                   ! MPI vector for 3-dimansional Real*4 Martix which nomal vector toward to East or West     
    lnlat=jon0-jos0+1
    lnlon=ioe0-iow0+1
    CALL MPI_TYPE_VECTOR((lnlat+ng*2)*k1,ng,lnlon+2*ng,MPI_INTEGER,my_mpi_vec,ierr)               
    CALL MPI_TYPE_COMMIT(my_mpi_vec,ierr)
    CALL MPI_SENDRECV(a(iow0,jos0-ng,1),1,my_mpi_vec,MPI_W,1,                 &
      a(ioe0+1,jos0-ng,1),1,my_mpi_vec,MPI_E,1,p_all_comm,istat,ierr)
    CALL MPI_SENDRECV(a(ioe0-ng+1,jos0-ng,1),1,my_mpi_vec,MPI_E,1,                &
      a(iow0-ng,jos0-ng,1),1,my_mpi_vec,MPI_W,1,p_all_comm,istat,ierr)
    CALL MPI_TYPE_FREE(my_mpi_vec,ierr)
  END SUBROUTINE exchangeEW3dInt
!!!!------------------------------------------------------
!!!  SUBROUTINE exchangeNS2dReal(a,iow0,ioe0,jos0,jon0,ng)
!!!    REAL(dp), INTENT(in out):: a(iow0-ng:ioe0+ng,jos0-ng:jon0+ng)
!!!    INTEGER, INTENT(in):: iow0,ioe0,jos0,jon0,ng
!!!    INTEGER:: my_mpi_vec    
!!!    CALL MPI_TYPE_VECTOR(1,(lnlon+2*ng)*ng,(lnlon+2*ng)*(lnlat+2*ng),MPI_REAL8,my_mpi_vec,ierr)
!!!    CALL MPI_TYPE_COMMIT(my_mpi_vec,ierr)    
!!!    CALL MPI_SENDRECV(a(iow0-ng,jos0),1,my_mpi_vec,MPI_S,1,                 &
!!!      a(iow0-ng,jon0+1),1,my_mpi_vec,MPI_N,1,p_all_comm,istat,ierr)
!!!    CALL MPI_SENDRECV(a(iow0-ng,jon0),1,my_mpi_vec,MPI_N,1,                &
!!!      a(iow0-ng,jos0-ng),1,my_mpi_vec,MPI_S,1,p_all_comm,istat,ierr)
!!!!!!    a(iow0-ing:iow0-1,:)=a(ioe0-ing+1:ioe0,:)
!!!!!!    a(ioe0+1:ioe0+ing,:)=a(iow0:iow0+ing-1,:)
!!!  END SUBROUTINE exchangeNS2dReal
!------------------------------------------------------
  SUBROUTINE exchangeNS3dReal(a,iow0,ioe0,jos0,jon0,ng,nh,lchangesign)
    ...
    INTEGER:: i,j,k,ih
    INTEGER:: lnlat,lnlon
    INTEGER:: my_mpi_vec   ! MPI vector for 3-dimansional Real*8 Martix which nomal vector toward to North or South
    lnlat=jon0-jos0+1
    lnlon=ioe0-iow0+1
    CALL MPI_TYPE_VECTOR(1,(lnlon+2*ng)*ng,(lnlon+2*ng)*(lnlat+2*ng),MPI_REAL8,my_mpi_vec,ierr)
    CALL MPI_TYPE_COMMIT(my_mpi_vec,ierr)
...
! ----------------------------------------------------------------------      
SUBROUTINE set_ghost_all()
  INTEGER:: I,J,K,ih
! --------------------------------
! SET PERIODIC BOUNDARY CONDITIONS (u2,v2,T2,S2 and tracers)
! ????? THIS ROUTINE will crash the MEMORY of fb1????????????????????
! --------------------------------
!!!  RETURN
  IF (.TRUE.) THEN
    CALL exchangeEW5dReal(fb2(:,:,:,:,:),1,lnlon,1,lnlat,ng,k1,jpocn+nocntrac+1,nh)
    CALL exchangeNS5dReal(fb2(:,:,:,:,:),1,lnlon,1,lnlat,ng,k1,jpocn+nocntrac+1,nh)
  ENDIF
! --------------------------------
! SET North/South/Equator BOUNDARY CONDITIONS (bjt)
! no flux for T,S, wall for U,V
! --------------------------------
  ...
  !! pe for N/S poles
    IF (lc%lLatPeriodical) THEN
      ...
      !  lnlat
      !  lnlat-1
      fb2(:,lnlat+1:lnlat+ng,:,1:2,1)=-fb2(:,lnlat:lnlat+1-ng:-1,:,1:2,1)    ! for u and v: change the signs and swap latitude dir
      fb2(:,lnlat+1:lnlat+ng,:,3:,1)=fb2(:,lnlat:lnlat+1-ng:-1,:,3:,1)       ! for the other variables (T,S,P) no sign change is needed
!!!      W(:,lnlat+1:lnlat+ng,:,1)=W(:,lnlat+1-ng:lnlat:,:,1)                 ! for W variable, no sign change is needed              
      ! S. Pole
      ! j  2
      !    1
      !  ============================== S. Pole
      !    0  <=      1
      !   -1  <=      2
      fb2(:,1-ng:0,:,1:2,2)=-fb2(:,ng:1:-1,:,1:2,2)    ! for u and v: change the signs and swap latitude dir
      fb2(:,1-ng:0,:,3:,2)=fb2(:,ng:1:-1,:,3:,2)       ! for the other variables
!!!      W(:,1-ng:0,:,2)=W(:,1:ng,:,2)                 ! for W variable      
    ELSE
      DO K=1,k1
        DO I=1,lnlon
          ih=2
          ! South Pole
          DO j=1-ng,0
            IF (IN(I,j,K,ih).EQ.1) THEN
            ! open boundary
!             W(I,j,K,ih)=W(I,1,K,ih)
              S2(I,j,K,ih)=MERGE(SSIT(I,j,K,ih),S2(I,1,K,ih),SSIT(I,j,K,ih).NE.xmissing)
              T2(I,j,K,ih)=MERGE(TSIT(I,j,K,ih),T2(I,1,K,ih),TSIT(I,j,K,ih).NE.xmissing)
              IF (louov) THEN
                U2(I,j,K,ih)=MERGE(USIT(I,j,K,ih),U2(I,1,K,ih),USIT(I,j,K,ih).NE.xmissing)
                V2(I,j,K,ih)=MERGE(VSIT(I,j,K,ih),V2(I,1,K,ih),VSIT(I,j,K,ih).NE.xmissing)
              ELSE
                U2(I,j,K,ih)=U2(I,1,K,ih)
                V2(I,j,K,ih)=V2(I,1,K,ih)
              ENDIF      
            ELSE
            ! wall
!!    !!         W(I,j,K,ih)=W(I,1,K,ih)
!!    !          S2(I,j,K,ih)=S2(I,1,K,ih)
!!    !          T2(I,j,K,ih)=T2(I,1,K,ih)
!!    !          U2(I,j,K,ih)=0._dp
!!    !          V2(I,j,K,ih)=0._dp
            ENDIF
!!!            P(I,j,K,ih)=P(I,1,K,ih)                          ! set in hydrostatic Eq.
            ! Open boundary
          ENDDO
          ih=1
          ! North Pole
          DO j=lnlat+1,lnlat+ng      
            IF (IN(I,j,K,ih).EQ.1) THEN
            ! open boundary
!             W(I,j,K,ih)=W(I,J,K,ih)
              S2(I,j,K,ih)=MERGE(SSIT(I,j,K,ih),S2(I,lnlat,K,ih),SSIT(I,j,K,ih).NE.xmissing)
              T2(I,j,K,ih)=MERGE(TSIT(I,j,K,ih),T2(I,lnlat,K,ih),TSIT(I,j,K,ih).NE.xmissing)
              IF (louov) THEN
                U2(I,j,K,ih)=MERGE(USIT(I,j,K,ih),U2(I,lnlat,K,ih),USIT(I,j,K,ih).NE.xmissing)
                V2(I,j,K,ih)=MERGE(VSIT(I,j,K,ih),V2(I,lnlat,K,ih),VSIT(I,j,K,ih).NE.xmissing)
              ELSE
                U2(I,j,K,ih)=U2(I,lnlat,K,ih)
                V2(I,j,K,ih)=V2(I,lnlat,K,ih)
              ENDIF
            ELSE
            ! wall
!!    !!      W(I,lnlat+1,K,ih)=W(I,J,K,ih)
!!    !       S2(I,lnlat+1,K,ih)=S2(I,lnlat,K,ih)
!!    !       T2(I,lnlat+1,K,ih)=T2(I,lnlat,K,ih)
!!    !       U2(I,lnlat+1,K,ih)=0._dp
!!    !       V2(I,lnlat+1,K,ih)=0._dp
            ENDIF
!!!            P(I,j,K,ih)=P(I,lnlat,K,ih)                          ! set in hydrostatic Eq.
            ! Open boundary
          ENDDO
        ENDDO
      ENDDO
  ...
  ! pe for Equator
  ! latitude exchange
    fb2(:,1-ng:0,:,:,1)=fb2(:,lnlat-ng+1:lnlat,:,:,2)
    fb2(:,lnlat+1:lnlat+ng,:,:,2)=fb2(:,1:ng,:,:,1)
!!!    W(:,1:ng,:,1)=W(:,lnlat-ng+1:lnlat,:,2)
!!!    W(:,lnlat+1:lnlat+ng,:,2)=W(:,1:ng,:,1)
  ! longitude exchange (It is exchanged in exchangeEW3dReal already)
  ELSE
  ENDIF
END SUBROUTINE set_ghost_all
! ----------------------------------------------------------------------
SUBROUTINE set_ghost_3d(a,nx,ny,ng,nh,lchangesign)
  ...
  INTEGER:: I,J,K,ih
  CALL exchangeEW3dReal(a(:,:,:),1,nx,1,ny,ng,nh)
...
!!!    IF ( lwarning_msg.GE.2 ) THEN
!!!      WRITE(nerr,*) p_pe,"set_ghost_4d 0.0"    
!!!      ! CALL p_barrier(p_all_! CALL p_barrier(p_all_comm) p_pe,"set_ghost_4d 1.0"    
!!!      IF ( (p_pe.EQ.3).AND.(lwarning_msg.GE.2) ) THEN
!!!        CALL ocn_msg()
!!!      ENDIF
!!!    ENDIF  
...
!!!    IF ( lwarning_msg.GE.2 ) THEN
!!!      ! CALL p_barrier(p_all_! CALL p_barrier(p_all_comm) p_pe,"set_ghost_4d 2.0"
!!!      IF ( (p_pe.EQ.3).AND.(lwarning_msg.GE.2) ) THEN
!!!        CALL ocn_msg()
!!!      ENDIF
!!!    ENDIF  
...
!!!    IF ( lwarning_msg.GE.2 ) THEN
!!!      ! CALL p_barrier(p_all_! CALL p_barrier(p_all_comm) p_pe,"set_ghost_4d 3.0"
!!!      IF ( (p_pe.EQ.3).AND.(lwarning_msg.GE.2) ) THEN
!!!        CALL ocn_msg()
!!!      ENDIF
!!!    ENDIF  
