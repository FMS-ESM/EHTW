MODULE mo_mpi
  ...
  PUBLIC :: p_parallel, p_parallel_io, p_parallel_ocean 
  ...
  LOGICAL :: p_parallel, p_parallel_io, p_parallel_ocean
...
!!LK  INTEGER :: iope                  ! PE able to do IO
  ...
  INTERFACE p_bcast
     ...
     MODULE PROCEDURE p_bcast_int_i2_2d
...
!!LK       ELSE
          ...
          p_io = 0
          p_ocean= 0
!!!          p_ocean=npes-1                        ! last processor for use for ocean model
...
!!!    p_ocean=npes-1                        ! last processor for use for ocean model
    ...
    END IF
     ...
     IF (npes < 2) THEN
       ...
       p_parallel_ocean = .TRUE.   ! can always do ocean model
       IF (mype == 0) THEN
    ...
    ELSE 
       ...
       ELSE
          p_parallel_io = .FALSE.
       END IF
       IF (mype == p_ocean) THEN
          p_parallel_ocean = .TRUE.
       ELSE
          p_parallel_ocean = .FALSE.
       END IF
       IF (mype == 0) THEN       
          WRITE (nerr,'(a,i4,a)') '  Run on ', npes, ' processors.'
          WRITE (nerr,'(a,i4)') '  p_io= ', p_io
       END IF
       IF (mype == p_ocean) THEN       
          WRITE (nerr,'(a,i4)') '  p_ocean= ', p_ocean
       END IF
       p_pe = mype
       p_nprocs = npes
    END IF
...
!!LK    END IF
    ...
    IF (p_parallel) THEN
       ...
       p_int_i2  = p_type (ii2, integer_type) 
...
!!!bjt   change the aborting error code from 0 to 3
...
! non-blocking receives
  ...
  SUBROUTINE p_bcast_int_i2_2d (buffer, p_source, comm)

    INTEGER*2, INTENT(inout) :: buffer(:,:)
    INTEGER, INTENT(in)    :: p_source
    INTEGER, OPTIONAL, INTENT(in) :: comm
    ...
    IF (npes == 1) THEN
       RETURN
    ELSE
       CALL MPI_BCAST (buffer, SIZE(buffer), p_int_i2, p_source, &
            p_comm, p_error)
    ENDIF
...
#endif
#endif
  END SUBROUTINE p_bcast_int_i2_2d
...
&                          flag, p_status, p_error)
  ...
  SUBROUTINE p_wait
    ...
    IF(lwarning_msg.GE.3) THEN
      WRITE(nerr,*) "I am in p_wait 1: MPI_WAIT, p_irequest=",p_irequest 
    ENDIF

    DO i = 1, p_irequest
      CALL MPI_WAIT(p_request(i), p_status, p_error)
      IF(lwarning_msg.GE.4) THEN
        WRITE(nerr,*) "p_pe=",p_pe,", I am in p_wait 2: MPI_WAIT, p_request(",i,")=",p_request(i)
        WRITE(nerr,*) "p_pe=",p_pe,", I am in p_wait 2: MPI_WAIT, p_status=",p_status
        WRITE(nerr,*) "p_pe=",p_pe,", I am in p_wait 2: MPI_WAIT, p_error=",p_error
        WRITE(nerr,*) "p_pe=",p_pe,", I am in p_wait 2: MPI_WAIT, p_request_alloc_size=",p_request_alloc_size
      ENDIF
    END DO 
    p_irequest = 0

    IF(lwarning_msg.GE.3) THEN
      WRITE(nerr,*) "I am in p_wait 3: leaving"
    ENDIF
