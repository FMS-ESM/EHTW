       ...
       DO
          ...
          IF (linp_tem) THEN
             IF (p_parallel_io) THEN
                ...
                ALLOCATE(zin2(nlev,2,nsp))
                zin2(:,:,:) = 0.0_dp
                IF (linp_lnp)       kkmax=ilev_t_min+ino_t_lev-2 !! if we nudge ln of surface pressure: ilev_t_max = ilev_t_min+ino_t_lev-2 
                IF (.NOT.linp_lnp)  kkmax=ilev_t_min+ino_t_lev-1 !! if we don't nudge ln of surface pressure: ilev_t_max = ilev_t_min+ino_t_lev-1   
