MODULE mo_scan_buffer
  ...
  REAL(dp), ALLOCATABLE :: sitwtte    (:,:,:)
  REAL(dp), ALLOCATABLE :: sitwste    (:,:,:)
  REAL(dp), ALLOCATABLE :: sitwute    (:,:,:)
  REAL(dp), ALLOCATABLE :: sitwvte    (:,:,:)
...
CONTAINS
  ...
  SUBROUTINE m_bufscan
    ...
    USE mo_control,       ONLY: lsit,locn,lwarning_msg,locn_msg    
    ...
    IF (lnot_used) THEN
       ...
       IF (lsit.AND.locn) THEN          
         ALLOCATE (sitwtte(nproma,lkvl+2,ngpblks))   ;sitwtte    = 0.0_dp
         ALLOCATE (sitwste(nproma,lkvl+2,ngpblks))   ;sitwste    = 0.0_dp
         ALLOCATE (sitwute(nproma,lkvl+2,ngpblks))   ;sitwute    = 0.0_dp
         ALLOCATE (sitwvte(nproma,lkvl+2,ngpblks))   ;sitwvte    = 0.0_dp
       ENDIF
