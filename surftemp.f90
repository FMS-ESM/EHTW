...
!               (+ downward, into the skin soil)
!           
!     PGRDFL  : GROUND HEAT FLUX  (W/m2) (+ upward, into the skin soil)
!
...
!     PGRDCAP : SURFACE areal HEAT CAPACITY (J/m2/K)
!
!     OUTPUT
!     ------
...
!     B.J. Tsuang, NCHU, June 2009:
!        MODIFY for inclduing SIT snow/ice/water skin temperature determination
...
!
     ...
     pqsnew(jl) = pqsold(jl) + zicp * pdqsold(jl)*(psnew(jl) -         &
                                                           psold(jl))
!!!bjt   ELSE
!!!bjt     psnew(jl) = psold(jl)
!!!bjt     pqsnew(jl) = pqsold(jl)
!!!bjt   END IF
!
 END DO
!
