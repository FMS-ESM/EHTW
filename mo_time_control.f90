MODULE mo_time_control
  ...
  USE mo_control,         ONLY: lcouple, lhd, nn, lnwp, nlev, ldiagamip, locn, lsit
  ...
  ! coupling with embedded ocean model (sit & ocn)


  LOGICAL               ::  ltrigsit = .TRUE.  ! logical for trigger one-column ocean model (sit_ocean)
  TYPE(time_event),SAVE    :: ev_trigsit
...
!!!       io_time_event(12,TIME_INC_HOURS,TRIG_FIRST,0) ! every 12 h

  LOGICAL               ::  ltrigocn = .FALSE.  ! logical for trigger embedded ocean
  TYPE(time_event),SAVE    :: ev_trigocn
  ...
  LOGICAL               ::  lreset_ocn_cpl = .FALSE.   ! just got data from the embedded ocean
!!!  LOGICAL               ::  lsice_nudg = .TRUE.    ! logical for spin up SIT/DIECAST
!!!  REAL(dp)              ::  spinup_time = 0._dp ! = 365._dp*86400._dp (1 yr)
    ! spinup time for nudging SIT/DIECAST (s) (default: 0 yr)
  ! ----------------------------------------------------------------------------
  ! subjob handling
...
!!    INTEGER           :: days, seconds
  ...
  SUBROUTINE init_events     !********* called in INITIALIZE *******************
    ...
    ELSEIF (lhd) THEN
      puthd = trigocn
      gethd = trigocn
!!!      WRITE(nerr,*) 'gethd%adjustment=',gethd%adjustment
!!!      WRITE(nerr,*) 'puthd%adjustment=',puthd%adjustment
      CALL message('',&
           'Synchronization of embedded 3D-ocean and hydrological discharge model done.')
    END IF
    ...
    IF (lsit) CALL echam_ev_init(ev_trigsit,trigsit,'trigger sit_ocean',  EV_TLEV_NEXT)
    IF (locn) CALL echam_ev_init(ev_trigocn,trigocn,'trigger embedded ocean',  EV_TLEV_NEXT)
...
!ik_bugfix_R1.02a
    ...
    ltrigsit = lsit .AND.(event_state(ev_trigsit, current_date).OR.lstart)
    !! trigger according to 1) lstart, and ev_trigocn
    
    ltrigocn = locn .AND.(event_state(ev_trigocn, current_date).OR.lstart.OR.(istep.LE.100))
    !! trigger according to 1) lstart, 2) istep <= 100, and ev_trigocn
!!!    lsice_nudg  = (istep.LE.NINT(spinup_time/delta_time))
!!!    lsice_nudg  = locn .AND. lsit .AND. (istep.LE.NINT(spinup_time/delta_time))
    !! trigger according to 1) lstart, 2) istep <= 100, and ev_trigocn
...
!      l2nd_day = next_date < my_day     ! new version
    ...
    IF (ldebugev) THEN
      ...
      IF (ltrigsit)     CALL echam_ev_print(ev_trigsit)
      IF (ltrigocn)     CALL echam_ev_print(ev_trigocn)
...
!
  ...
  REAL(dp) FUNCTION set_delta_time (truncation,nlevel)
    ...
    ELSE IF (nlevel == 31) THEN
      ...
      ELSE IF (truncation ==511) THEN; set_delta_time =   60.0_dp
      ELSE
       CALL finish ('set_delta_time', 'Truncation not supported.')
      END IF
