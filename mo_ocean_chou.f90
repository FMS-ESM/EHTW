...
!!!                                MPI_INTEGER,MPI_STATUS_SIZE,p_all_comm,p_real_dp
  ...
  REAL(dp), PARAMETER:: HBK0=1.34E-7_dp                     ! molecular heat diffusivity (m2/s) (=1.34E-3_dp cm2/s)
                                                            ! (Paulson and Simpson, 1981; Chia and pwu, 1998; Mellor and Durbin, 1975)  
  REAL(dp), PARAMETER:: DM0Z=1.E5_dp                        ! near shore momentum diffusivity (m2/s) for preventing generating  near-shore vortex
                                                            ! Noisy for current                                   
                                                            ! REAL(dp), PARAMETER:: DM0Z=1.E2_dp (seems to small)
                                                            ! REAL(dp), PARAMETER:: DM0Z=5.E2_dp (seems to small) (2012/12/09) (v9.6)
                                                            ! REAL(dp), PARAMETER:: DM0Z=1.E4_dp (seems to small) (2012/12/09)
                                                            ! REAL(dp), PARAMETER:: DM0Z=5.E4_dp (OK for current, but still produce large current near shore) (2012/12/10)  (v9.7)                                
                                                            ! REAL(dp), PARAMETER:: DM0Z=1.E5_dp (OK for current, but still produce large current in Indian Ocean) (2012/12/15) (v9.7)
                                                            !                                     too mush for depth at 150 m) (2012/12/9)                                  
                                                            ! REAL(dp), PARAMETER:: DM0Z=1.E6_dp (crash, too large velocity) (2012/12/15) (v9.7)
                                                            ! REAL(dp), PARAMETER:: DM0Z=5.E6_dp (crash, too large velocity) (2012/12/15) (v9.7)
                                                            ! REAL(dp), PARAMETER:: DM0Z=2.E5_dp (crash, WARNING! high wind speed: 232. m/s) (2012/12/16) (v9.7) 
                                                            ! REAL(dp), PARAMETER:: DM0Z=2.E5_dp (Strong filter, crash, too large velocity) (2012/12/16) (v9.7)
                                                            ! REAL(dp), PARAMETER:: DM0Z=1.E5_dp (crash, Strong filter, crash, too large velocity) (2012/12/16) (v9.7)
...
!
    IF (lstart.AND.(ocn_couple_option.NE.13)) THEN
      ...
      IF(lwarning_msg.GE.2) then
        CALL p_barrier(p_all_comm)    
        IF ( p_parallel_ocean ) WRITE(nerr,*) p_pe,"run_ocean 1.12"
        IF ( (p_pe.EQ.3).AND.(lwarning_msg.GE.3) ) THEN
    ...
    IF(lwarning_msg.GE.2) then
      CALL p_barrier(p_all_comm)    
      IF ( p_parallel_ocean ) WRITE(nerr,*) p_pe,"run_ocean 1.13"
      IF ( (p_pe.EQ.3).AND.(lwarning_msg.GE.3) ) THEN
...
!
    ...
    IF(lwarning_msg.GE.2) then
      CALL p_barrier(p_all_comm)    
      IF ( p_parallel_ocean ) WRITE(nerr,*) p_pe,"run_ocean 1.14"
      IF ( (p_pe.EQ.3).AND.(lwarning_msg.GE.3) ) THEN
    ...
    IF(lwarning_msg.GE.2) then
      CALL p_barrier(p_all_comm)    
      IF ( p_parallel_ocean ) WRITE(nerr,*) p_pe,"run_ocean 1.15"
      IF ( (p_pe.EQ.3).AND.(lwarning_msg.GE.3) ) THEN
...
!    
    ...
    IF ( lwarning_msg.GE.2 ) THEN
      CALL p_barrier(p_all_comm)    
      IF ( p_parallel_ocean ) WRITE(nerr,*) p_pe,"run_ocean 4.1: o2a: ocnt,ocnu,ocnv,ocnw"
    END IF
...
!!!              CALL intpol_o2a(MERGE(DBLE(KVH2(:,:,jk,:)),xmissing,IW(:,:,jk+1,:).EQ.1),gl_ocnkvh(:,jk,:),0._dp,1._dp,2,ng)     ! from m2/s to m2/s
    ...
    IF(lwarning_msg.GE.2) then
      CALL p_barrier(p_all_comm)
      IF ( p_parallel_ocean ) WRITE(nerr,*) p_pe,"run_ocean 5.0: Put data to individual nodes"
      IF ( (p_pe.EQ.3).AND.(lwarning_msg.GE.3) ) THEN
...
!  
  ...
  !
    IF ( lwarning_msg.GE.2 ) THEN
      CALL p_barrier(p_all_comm)
      IF (p_parallel_ocean) THEN
        WRITE(nerr,*) p_pe,"ocn_stepon 1.0"
        CALL FLUSH(nerr)
      ENDIF        
    ENDIF
    ...
    IF ( lwarning_msg.GE.2 ) THEN
      CALL p_barrier(p_all_comm)
      IF (p_parallel_ocean) THEN
        WRITE(nerr,*) p_pe,"ocn_stepon 1.5: ocn_dt=",ocn_dt,"max_vel_water=",max_vel_water
        CALL FLUSH(nerr)
      ENDIF        
    ENDIF
    ...
    IF ( lwarning_msg.GE.2 ) THEN
      CALL p_barrier(p_all_comm)
      IF (p_parallel_ocean) THEN
        WRITE(nerr,*) p_pe,"ocn_stepon 1.6: two_dt_old=",two_dt_old,"s, two_dt=",two_dt,"s"
        WRITE(nerr,*) p_pe,"ocn_stepon 1.6: n_ocn_step=",n_ocn_step
        CALL FLUSH(nerr)
      ENDIF        
    ENDIF
    IF ((two_dt_old.NE.two_dt).OR.lfirst_cycle) THEN
      IF ( lwarning_msg.GE.2 ) THEN
        CALL p_barrier(p_all_comm)
        IF (p_parallel_ocean) THEN
          WRITE(nerr,*) p_pe,"ocn_stepon 1.7"
          CALL FLUSH(nerr)
        ENDIF
      ENDIF
      ...
      IF ( lwarning_msg.GE.2 ) THEN
        CALL p_barrier(p_all_comm)
        IF (p_parallel_ocean) THEN
          WRITE(nerr,*) p_pe,"ocn_stepon 1.8"
          CALL FLUSH(nerr)
        ENDIF
        IF ( lwarning_msg.GE.2 ) THEN        
      ...
      IF ( lwarning_msg.GE.2 ) THEN
        CALL p_barrier(p_all_comm)
        IF (p_parallel_ocean) THEN
          WRITE(nerr,*) p_pe,"ocn_stepon 1.9"
          CALL FLUSH(nerr)
        ENDIF
        IF ( lwarning_msg.GE.2 ) THEN        
    ...
    IF (p_parallel_ocean) THEN
      WRITE(nerr,1107) istep,max_vel_land,max_vel_water,INT(ocn_dt)
  ...
  !
    ...
    IF ( lwarning_msg.GE.2 ) THEN    
      CALL p_barrier(p_all_comm)
      IF (p_parallel_ocean) THEN
        WRITE(nerr,*) p_pe,"set_ocn_dt 2.0"
      !!! CALL ocn_msg()
        CALL FLUSH(nerr)
      ENDIF
    ENDIF    
...
!!!    ENDIF
 ...
 14 FORMAT('LONGITUDE(X) GRID DIMENSION:',I4/    &
    ...
    !!! IF ( lwarning_msg.GE.2 ) WRITE(nerr,244) (ih,(j,(KB(i+1-2,j,ih),i=31,47),j=lnlat+ng,1-ng,-1),ih=1,nh)
  ...
  END SUBROUTINE PREP
    ...
    ! ======================================================================
      ...
      IF (p_parallel_ocean) THEN
!!!      IF (MYID .EQ. 0)  THEN
        IF ( p_parallel_ocean .AND. lwarning_msg.GE.2 )  WRITE(nerr,*) p_pe,"INMETS 1: pe=",p_pe      
        IF (.TRUE.) THEN
...
!!!      REWIND 10
        ...
        ! New single record input
          ...
          IF ( p_parallel_ocean .AND. lwarning_msg.GE.2 )  WRITE(nerr,*) "METERS=",METERS(1,1)
        ELSE
        !!! etopo1 netcdf file
        !!!  ISTART(1) = 1
        !!!  ISTART(2) = 1
        ...
        ENDIF
      ENDIF
      IF ( p_parallel_ocean .AND. lwarning_msg.GE.2 )  WRITE(nerr,*) p_pe,"INMETS 2: pe=",p_pe
      CALL p_bcast(METERS,p_ocean)
      !!! CALL MPI_BCAST(METERS,NX*NY,MPI_REAL,0,p_all_comm,ierr)
      IF ( p_parallel_ocean .AND. lwarning_msg.GE.2 )  WRITE(nerr,*) p_pe,"INMETS 3: pe=",p_pe,"METERS=",METERS(1,1)
      ! ==============================================================
 ...
 101        IF (depth(I,J,ih).GT.ocn_tlz) N=N+1
      ...
      CALL p_barrier(p_all_comm)
      t_mpi_barrier=t_mpi_barrier+MPI_WTIME()-t_tmp1
      IF ( p_parallel_ocean .AND. lwarning_msg.GE.2 )  WRITE(nerr,*) p_pe,"INMETS 3.3: pe=",p_pe,"lnlon=",lnlon,"lnlat=",lnlat
...
SUBROUTINE ocn_init
  ...
  ! ----------------------------------------------------------------------
    ...
    INTEGER:: I,J,K,M,N,NM,MP,N2,N4,N6,N8,ii,jj,ih,NL,L,LL,LLL
          ...
          ! cell faces two grid intervals from shore
            DO j=1,lnlat
              ...
              DO I=1,lnlon-1
                ...
                NL=N2
                DO LL=1,L
                  NL=IN(I-L,J,K,ih)*NL*IN(I+L+1,J,K,ih)
                ENDDO
                DMX0(I,J,K,ih)=DMX0(I,J,K,ih)+(1.-NL)*CARPET
                ! DMX0(I,J,K,ih)=DMX0(I,J,K,ih)+N2*(1.-NL)*CARPET
              ENDDO
            ENDDO
            ! DO J=L,lnlat-L
            DO J=1,lnlat-1
              DO i=1,lnlon
                ...
                NL=N2
                DO LL=1,L
                  NL=IN(I,J-L,K,ih)*NL*IN(I,J+L+1,K,ih)
                ENDDO
                DMY0(I,J,K,ih)=DMY0(I,J,K,ih)+(1.-NL)*CARPET
                ! DMY0(I,J,K,ih)=DMY0(I,J,K,ih)+N2*(1.-NL)*CARPET
              ENDDO
            ENDDO
            CARPET=TMP*CARPET
          ENDDO
...
!!!            DSE=SDBL(IP,J,K)
      ...
      CALL p_barrier(p_all_comm)    
      t_mpi_barrier=t_mpi_barrier+MPI_WTIME()-t_tmp1
      IF ( p_parallel_ocean .AND. lwarning_msg.GE.2 )  WRITE(nerr,*) p_pe,"indata 5.0."
      ...
      CALL p_barrier(p_all_comm)    
      t_mpi_barrier=t_mpi_barrier+MPI_WTIME()-t_tmp1
      IF ( p_parallel_ocean .AND. lwarning_msg.GE.2 )  WRITE(nerr,*) p_pe,"indata 5.1."
      CALL FLUSH(nerr)
      ! ===============================================
...
!!!    ENDIF
    ...
    DEALLOCATE (XVDEG)
    DEALLOCATE (XDEG)
    IF ( p_parallel_ocean .AND. lwarning_msg.GE.2 )  WRITE(nerr,*) p_pe,"cleanup_ocean 3.1"
    DEALLOCATE (CS)
    DEALLOCATE (CSV)
    DEALLOCATE (OCS)
!!!!    DEALLOCATE (OCSV)
    IF ( p_parallel_ocean .AND. lwarning_msg.GE.2 )  WRITE(nerr,*) p_pe,"cleanup_ocean 3.2"
    
    DEALLOCATE (DX)
    IF ( p_parallel_ocean .AND. lwarning_msg.GE.2 )  WRITE(nerr,*) p_pe,"cleanup_ocean 3.2.1"
    DEALLOCATE (DXV)
    IF ( p_parallel_ocean .AND. lwarning_msg.GE.2 )  WRITE(nerr,*) p_pe,"cleanup_ocean 3.2.2"
    DEALLOCATE (ODX)
    IF ( p_parallel_ocean .AND. lwarning_msg.GE.2 )  WRITE(nerr,*) p_pe,"cleanup_ocean 3.2.3"
    DEALLOCATE (ODXV)
    IF ( p_parallel_ocean .AND. lwarning_msg.GE.2 )  WRITE(nerr,*) p_pe,"cleanup_ocean 3.3"
    
    DEALLOCATE (DY)
    DEALLOCATE (DYV)
    DEALLOCATE (ODY)
    DEALLOCATE (ODYV)
    IF ( p_parallel_ocean .AND. lwarning_msg.GE.2 )  WRITE(nerr,*) p_pe,"cleanup_ocean 4.0"
    !  
    ! "A" grid arrays
    !
    IF ( p_parallel_ocean .AND. lwarning_msg.GE.2 )  WRITE(nerr,*) p_pe,"cleanup_ocean 5.0"
    ! Logical depth KB and associated masking arrays
    ! Lower precision logical depth and masking arrays
    ! "A" grid arrays
    !
    ...
    IF ( p_parallel_ocean .AND. lwarning_msg.GE.2 )  WRITE(nerr,*) p_pe,"cleanup_ocean: 6.0"
    ...
    IF ( lwarning_msg.GE.2 ) THEN
      CALL p_barrier(p_all_comm)    
      WRITE(nerr,*) p_pe,"cleanup_ocean: 7.0"
    ENDIF   
    DEALLOCATE (F)
    IF ( lwarning_msg.GE.2 ) THEN
      CALL p_barrier(p_all_comm)    
      WRITE(nerr,*) p_pe,"cleanup_ocean: 7.1"
    ENDIF    
    DEALLOCATE (TANPHI)
    ...
    IF ( lwarning_msg.GE.2 ) THEN
      CALL p_barrier(p_all_comm)    
      WRITE(nerr,*) p_pe,"cleanup_ocean: 7.2"
    ENDIF
     DEALLOCATE (IU0)
    IF ( lwarning_msg.GE.2 ) THEN
      CALL p_barrier(p_all_comm)    
      WRITE(nerr,*) p_pe,"cleanup_ocean: 7.3"
    ENDIF    
    DEALLOCATE (IU)
    IF ( lwarning_msg.GE.2 ) THEN
      CALL p_barrier(p_all_comm)    
      WRITE(nerr,*) p_pe,"cleanup_ocean: 7.4"
    ENDIF    
    DEALLOCATE (U)
    
    IF ( lwarning_msg.GE.2 ) THEN
      CALL p_barrier(p_all_comm)    
      WRITE(nerr,*) p_pe,"cleanup_ocean: 7.5"
    ENDIF
        
    DEALLOCATE (IV0)
    IF ( lwarning_msg.GE.2 ) THEN
      CALL p_barrier(p_all_comm)    
      WRITE(nerr,*) p_pe,"cleanup_ocean: 7.6"
    ENDIF        
    DEALLOCATE (IV)
    IF ( lwarning_msg.GE.2 ) THEN
      CALL p_barrier(p_all_comm)    
      WRITE(nerr,*) p_pe,"cleanup_ocean: 7.7"
    ENDIF
    DEALLOCATE (V)
    ...
    IF ( lwarning_msg.GE.2 ) THEN
      CALL p_barrier(p_all_comm)
      WRITE(nerr,*) p_pe,"cleanup_ocean: 7.9"
    ENDIF
    DEALLOCATE (W)
    ...
    IF ( lwarning_msg.GE.2 ) THEN
      CALL p_barrier(p_all_comm)
      WRITE(nerr,*) p_pe,"cleanup_ocean: 9.0"
    ENDIF
    ...
    IF ( lwarning_msg.GE.2 ) THEN
      CALL p_barrier(p_all_comm)
      WRITE(nerr,*) p_pe,"cleanup_ocean: 10.0"
    ENDIF
    !   
    DEALLOCATE (DMX)
    DEALLOCATE (DMY)
    ...
    IF ( lwarning_msg.GE.2 ) THEN
      CALL p_barrier(p_all_comm)    
      WRITE(nerr,*) p_pe,"cleanup_ocean: 11.0"
    ENDIF  
...
!        IF ( lwarning_msg.GE.2 ) THEN
!          CALL p_barrier(p_all_comm)
!          WRITE(nerr,*) p_pe,"P_BICGSTAB_LE 8.0, rhn",rhn              
!          WRITE(nerr,*) p_pe,"P=",P
!          WRITE(nerr,*) p_pe,"w=",w
!          WRITE(nerr,*) p_pe,"V=",V
!          IF ( (p_pe.EQ.3).AND.(lwarning_msg.GE.3) ) THEN
...
!        IF ( lwarning_msg.GE.2 ) THEN
!          CALL p_barrier(p_all_comm)
!          WRITE(nerr,*) p_pe,"P_BICGSTAB_LE 8.1"    
!          IF ( (p_pe.EQ.3).AND.(lwarning_msg.GE.3) ) THEN
!            CALL ocn_msg()
!          ENDIF       
!        ENDIF 
...
!        IF ( lwarning_msg.GE.2 ) THEN
!          CALL p_barrier(p_all_comm)
!          WRITE(nerr,*) p_pe,"P_BICGSTAB_LE 8.2"    
!          IF ( (p_pe.EQ.3).AND.(lwarning_msg.GE.3) ) THEN
!            CALL ocn_msg()
!          ENDIF       
!        ENDIF       
...
!        IF ( lwarning_msg.GE.2 ) THEN
!          CALL p_barrier(p_all_comm)
!          WRITE(nerr,*) p_pe,"P_BICGSTAB_LE 8.3"    
!          IF ( (p_pe.EQ.3).AND.(lwarning_msg.GE.3) ) THEN
!            CALL ocn_msg()
!          ENDIF       
!        ENDIF       
...
!        IF ( lwarning_msg.GE.2 ) THEN
!          CALL p_barrier(p_all_comm)
!          WRITE(nerr,*) p_pe,"P_BICGSTAB_LE 9.0"    
!          IF ( (p_pe.EQ.3).AND.(lwarning_msg.GE.3) ) THEN
!            CALL ocn_msg()
!          ENDIF       
!        ENDIF 
