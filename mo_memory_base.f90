...
!------------------------------------------------------------------------------
  ...
  USE mo_linked_list,  ONLY: memory_info, memory_type, t_stream, list_element,&
                             ...
                             SNOWICE, SNOWICE2, WATER, WATERF, OCEAN, OCEANF, &
...
!------------------------------------------------------------------------------
  ...
  PUBLIC :: GAUSSIAN, FOURIER, SPECTRAL, HEXAGONAL, GRIDPOINT, OGAUSSIAN ! flag 'repr'
  PUBLIC :: BELOWSUR, SURFACE, ABOVESUR2, ABOVESUR10, &                  ! 'level_type'
            HYBRID, HYBRID_H, SNOWICE, SNOWICE2, WATER, WATERF, &        !                                  !
            OCEAN, OCEANF
  PUBLIC :: UNKNOWN
      ...
      IF (info% repr == GAUSSIAN) THEN
        ...
        IF (iklat > 0) THEN
          ...
          IF (info% IO_var_indx(iklat) < 0) info% IO_var_indx(iklat) = &
            IO_get_varindx('lat')
        ENDIF
      ELSEIF (info% repr == OGAUSSIAN) THEN
        IF (iklon > 0) THEN
          IF (info%  DIM       (iklon) < 0) info%  DIM (iklon) = ldc% ngolon
          IF (info% gdim       (iklon) < 0) info% gdim (iklon) = ldc% nolon
          IF (info% IO_var_indx(iklon) < 0) info% IO_var_indx(iklon) = &
            IO_get_varindx('olon')
        ENDIF
        
        IF (iklat > 0) THEN
          IF (info%  DIM       (iklat) < 0) info%  DIM (iklat) = ldc% ngolat
          IF (info% gdim       (iklat) < 0) info% gdim (iklat) = ldc% nolat
          IF (info% IO_var_indx(iklat) < 0) info% IO_var_indx(iklat) = &
            IO_get_varindx('olat')
        ENDIF
