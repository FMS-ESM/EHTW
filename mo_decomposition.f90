MODULE mo_decomposition
 ...
 !!!  INTEGER           :: nlatx ! number of latitudes to exchange with pe_x
 !!!
 !!!  INTEGER           :: nlon  ! number of longitudes on this pe
 !!!  INTEGER           :: lone  ! eastern longitude  on this pe (0=1,N=nlon)
 !!!  INTEGER           :: lonw  ! western longitude on this pe
 !!!  INTEGER           :: pe_e  ! processor id of neighbour in the east
 !!!  INTEGER           :: pe_w  ! processor id of neighbour in the west
 !!!END TYPE pe_timcom
  ...
  TYPE pe_decomposed
     ...
     ! ocean
     INTEGER          :: nolon    ! number of global ocean columns in longitude direction
     INTEGER          :: nolat    ! number of global ocean rows in latitude direction
     INTEGER          :: nolev    ! number of levels in ocean domain
     LOGICAL:: lLonPeriodical           ! .TRUE. = periocal in longitude direction
     LOGICAL:: lLatPeriodical           ! .TRUE. = periocal in latitude direction    
     REAL(dp):: lonw0                   ! lonw0 is western most longitude degrees (west cell face of I=1 (=2) zone)
                                        ! longitude coordinate increases eastward
                                        ! -? < lonw0 < 360
     REAL(dp):: lone0                   ! lone0 is eastern most longitude degree (east cell face of I=ioe0 zone)
                                        ! longitude coordinate increases eastward
                                        ! -? < lone0 < 720, and lone0 > lonw0
     REAL(dp):: lats0                   ! lats0 is the southmost face latitude degree
     REAL(dp):: latn0                   ! latn0 is the northmost face latitude degree
     INTEGER:: jas0                     ! southmost index of ECHAM philat of ocean domain (global index,
                                        ! (ocn_yvdeg(0) = -90. at the South Pole, increasng from south to north)  
     INTEGER:: jan0                     ! northhmost index of ECHAM philat of ocean domain
     INTEGER:: iaw0                     ! westmost index of ECHAM philon of ocean domain    
     INTEGER:: iae0                     ! easttmost index of ECHAM philon of ocean domain
     ...
     ! Ocean Gaussian (OGAUSSIAN) grid

     INTEGER          :: ngolat     ! number of ocean latitudes  on PE
     INTEGER          :: ngolon     ! number of ocean longitudes on PE
     INTEGER          :: ngolpx     ! number of ocean longitudes allocated
     INTEGER          :: ngolh(2)   ! number of ocean latitudes on each hemisphere
     INTEGER          :: golats(2)  ! start values of ocean latitudes
     INTEGER          :: golate(2)  ! end values of ocean latitudes
     INTEGER          :: golons(2)  ! start values of ocean longitudes
     INTEGER          :: golone(2)  ! end values of ocean longitudes
     INTEGER ,POINTER :: golat(:)   ! global ocean latitude index N->S
     INTEGER ,POINTER :: golon(:)   ! offset to global ocean longitude
     !
     INTEGER:: igow0                    ! first global ocean i index (=2 default)
     INTEGER:: igoe0                    ! last global ocean i index (=1+lnlon-1)
     INTEGER:: jgos0                    ! first global ocean j index (=2 default)
     INTEGER:: jgon0                    ! last global ocean j index (=jos0+lnlat-1)
     
     REAL(dp):: pelonw0(nh)             ! pelonw0 is western most longitude degrees of each pe
     REAL(dp):: pelone0(nh)             ! pelone0 is eastern most longitude degree of each pe
     REAL(dp):: pelats0(nh)             ! pelats0 is the southmost face latitude degree of each pe
     REAL(dp):: pelatn0(nh)             ! pelatn0 is the northmost face latitude degree of each pe
     
     INTEGER:: iow0(nh)=2               ! first ocean i index (=2 default)
     INTEGER:: ioe0(nh)                 ! last ocean i index (=iow0+lnlon-1)
     INTEGER:: jos0(nh)=2               ! first ocean j index (=2 default)
     INTEGER:: jon0(nh)                 ! last ocean j index (=jos0+lnlat-1)
     INTEGER:: llnlon(nh)               ! number of longitudes on this pe in each hemosphere(1 for N.H., 2 for S. H.)
     INTEGER:: llnlat(nh)               ! number of latitudes on this pe in each hemosphere(1 for N.H., 2 for S. H.)
...
CONTAINS
  ...
  INTEGER  ,OPTIONAL   ,INTENT(in)    :: lons_1d ! longitude for column model
    ...
    ELSE
      ...
      ALLOCATE (pe_dc% glon( pe_dc% nglat))                              ! bjt, need to be checked
      pe_dc% glon(1:pe_dc% nglat/2)  = pe_dc% glons(1)-1                 ! bjt, need to be checked
      pe_dc% glon(pe_dc% nglat/2+1:) = pe_dc% glons(2)-1                 ! bjt, need to be checked
    ENDIF
    !
  ...
  SUBROUTINE print_decomposition (dc)
    ...
    USE mo_control,     ONLY: locn   ! bjt,2012/1/1
    ...
    IF (locn) THEN
      WRITE (nout,'(i5,a)') dc%ngolat, ' latitudes  in grid space'
      WRITE (nout,'(i5,a)') dc%ngolon, ' longitudes in grid space'
      !!! WRITE (nout,'(i5,a)') dc%nglpx, ' longitudes allocated'
      WRITE (nout,'(2i5,a)')dc%ngolh,  ' latitudes on N/S hemisphere'
      WRITE (nout,'(i5,a)') dc%nglat*dc%ngolon, ' grid points (total)'
      WRITE (nout,'(4(a,i5))') ' golatse: ', dc%golats(1), ' -', dc%golate(1), &
           ',   ', dc%golats(2), ' -', dc%golate(2)
      WRITE (nout,'(4(a,i5))') ' golonse: ', dc%golons(1), ' -', dc%golone(1), &
           ',   ', dc%golons(2), ' -', dc%golone(2)
      WRITE (nout,'(a,12i5/(9x,12i5))') ' golat    : ', dc% golat
      WRITE (nout,'(a,12i5/(9x,12i5))') ' golon    : ', dc% golon
      WRITE (nout,*)
    ENDIF
