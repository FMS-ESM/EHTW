...
!BOX
      ...
      IF (p_parallel_io) THEN
        ...
        !!! iunit = position_nml ('NDGCTL', inml, status=ipos)
        !!! SELECT CASE (ipos)
        !!! CASE (POSITIONED);  READ (iunit,ndgctl)
        !!! END SELECT
      ENDIF
...
!!$         !>>dod bugfix
!!$         WRITE(mess, '(a31,i0)') ' Correlation results on unit  ', ndunit
!!$         CALL message(mess, ' file: '//TRIM(cfile))
!!$         !<<dod
